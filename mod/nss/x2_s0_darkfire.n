...
void AddFlamingEffectToWeapon(object oTarget, float fDuration, int nCasterLvl) {
	...
	IPSafeAddItemProperty(oTarget, ItemPropertyDamageBonus(IP_CONST_DAMAGETYPE_FIRE, IP_CONST_DAMAGEBONUS_1d4),
		fDuration, X2_IP_ADDPROP_POLICY_KEEP_EXISTING);
	IPSafeAddItemProperty(oTarget, ItemPropertyVisualEffect(ITEM_VISUAL_FIRE), fDuration,
...
void main() {
	...
	if ( GetIsObjectValid(oMyWeapon) ) {
		...
		if ( nDuration > 0 ) {
			...
			ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eDur, GetItemPossessor(oMyWeapon), TurnsToSeconds(
					nDuration));
