...
float fMaxRange = 55.0f; //Max Range
float fMinRange = 5.0f; //Min Range
float fWidth = 40.0f; //Radius of cone at widest point
int iDC = 12; //Difficulty check(reflex save for half damage)
	...
	switch ( iState ) {
		case 100: //Ready
			if ( AcquireTarget(oUser) ) {
				ApplyDamage();
				if ( GetIsPC(oUser) )
					SendMessageToPC(oUser, "Gefeuert auf " + GetLocalString(OBJECT_SELF, "sTarget"));
			}
			break;
		case 110: //Reload
			if ( GetIsPC(oUser) ) {
				SendMessageToPC(oUser, "Ballista wieder laden");
			}
		...
		case TRUE: //If User is a PC, send reload messages.
			{
				if ( ( !GetIsInCombat(oUser) ) && ( GetDistanceToObject(oUser) < 2.5 ) ) {
					SendMessageToPC(oUser, "Ballista geladen");
					SetLocalInt(OBJECT_SELF, "iState", ST_READY);
				} else {
					SendMessageToPC(oUser, "Beladen unterbrochen");
...
int AcquireTarget(object oUser) {
	...
	oTarget = GetFirstObjectInShape(SHAPE_CONE, fWidth, lTarget, TRUE, OBJECT_TYPE_CREATURE, GetPosition(
					  OBJECT_SELF));
	while ( GetIsObjectValid(oTarget) ) {
		if ( GetIsAcceptableTarget(oUser, oTarget)
			&& GetDistanceBetween(oTarget, OBJECT_SELF) > fMinRange
			&& GetDistanceBetween(oTarget, OBJECT_SELF) <= fMaxRange
			&& GetObjectSeen(oTarget, oUser) && CheckAngleToTarget(oTarget, fAngle, fOrientation) ) {
		...
		oTarget = GetNextObjectInShape(SHAPE_CONE, fWidth, lTarget, TRUE, OBJECT_TYPE_CREATURE, GetPosition(
						  OBJECT_SELF));
	}
	if ( GetIsPC(oUser) )
		SendMessageToPC(oUser, "Du kannst keinen sauberen Schuss auf den Feind abgeben!");
	return FALSE;
}
...
void ApplyDamage() {
	...
	while ( GetIsObjectValid(oTarget) ) {
		...
		iDamage = d6(iDice); //Set Ballista Damage Here
		eDam = EffectDamage(iDamage, DAMAGE_TYPE_BLUDGEONING, DAMAGE_POWER_NORMAL);
		iDamage = GetReflexAdjustedDamage(iDamage, oTarget, iDC, SAVING_THROW_REFLEX);
		DelayCommand(fDelay, ApplyEffectToObject(DURATION_TYPE_INSTANT, eDam, oTarget));
		//DelayCommand(fDelay, PlaySound("as_cv_boomdist2"));
		DelayCommand(fDelay, ApplyEffectAtLocation(DURATION_TYPE_INSTANT, eFire, lTarget));
...
int GetIsAcceptableTarget(object oUser, object oTarget) {
	...
	int iFriends = 0;
	int iEnemies = 0;
	oPotentialVictim = GetFirstObjectInShape(SHAPE_SPHERE, fSize, GetLocation(oTarget), TRUE,
	...
	while ( GetIsObjectValid(oPotentialVictim) ) {
		if ( GetIsEnemy(oTarget, oUser) )
			...
			iFriends += 1;
		oPotentialVictim = GetNextObjectInShape(SHAPE_SPHERE, fSize, GetLocation(oTarget), TRUE,
							   OBJECT_TYPE_CREATURE);
	}
...
int CheckAngleToTarget(object oTarget, float fAngle, float fOrientation) {
	...
	if ( fTargetAngle >= fRightHeading && fTargetAngle <= fLeftHeading ) {
		PrintString("Winkel in Reichweite");
		return 1;
	} else
		return 0;
}
