...
void SJ_TileMagic_AddStyleToPalette(object oPalette, string sStyle, int nTile, float fZ = 0.0, int bNormalise = TRUE,
									int nRotation = 0);
...
//  - bNormalise:   TRUE (to compensate for tile's internal offset) or FALSE
//  - nRotation:    SJ_ROTATION_* constant
void SJ_TileMagic_CoverBorder(object oArea, int nTile, int nWidth = 1, float fZ = 0.0, int bNormalise = TRUE,
							  int nRotation = 0);
...
//  - bNormalise:   TRUE (to compensate for tile's internal offset) or FALSE
//  - nRotation:    SJ_ROTATION_* constant
void SJ_TileMagic_CoverRange(object oArea, int nTile, int nColumn = 0, int nRow = 0, int nColumns = 1, int nRows = 1, float fZ = 0.0, int bNormalise = TRUE,
							 int nRotation = 0);
...
void SJ_TileMagic_CoverSpot(object oArea, int nTile, float fX, float fY, float fZ = 0.0, int bNormalise = TRUE,
							float Rotation = 0.0);
...
//  - bNormalise:   TRUE (to compensate for tile's internal offset) or FALSE
//  - nRotation:    SJ_ROTATION_* constant
void SJ_TileMagic_CoverTile(object oArea, int nTile, int nColumn, int nRow, float fZ = 0.0, int bNormalise = TRUE,
							int nRotation = 0);
...
void TLChangeAreaGroundTileRange(object oArea, int nTile, int nColumn, int nRow, int nColumns = 1, int nRows = 1,
								 float fZ = -0.4) {
...
void SJ_TileMagic_AddRowToMatrix(object oMatrix, int nIndex, string sRow) {
	...
	if ( sRow == "" ) {
		...
		sDebug = "SJ_TileMagic_AddRowToMatrix: failled as row ( " +
				 IntToString(nIndex) + ") exceeded lower or upper bounds.";
		...
		// check if row exists
		sDebug = "SJ_TileMagic_AddRowToMatrix: failled as row ( " +
				 sIndex + ") already exisited in datapoint.";
	}
...
void SJ_TileMagic_AddStyleToPalette(object oPalette, string sStyle, int nTile, float fZ = 0.0, int bNormalise = TRUE,
	...
	if ( sStyle == "" ) {
		...
		// check if style is a single character
		sDebug = "SJ_TileMagic_AddStyleToPalette: failled as style ( " +
				 sStyle + ") exceeded 1 character and is invalid.";
		...
		sDebug = "SJ_TileMagic_AddStyleToPalette: failled as style ( " +
				 sStyle + ") already exisited in datapoint.";
	}
...
void SJ_TileMagic_ClearBorder(object oArea, int nWidth = 1) {
	...
	if ( nWidth < 1 ) {
		// log an error
		string sDebug = "SJ_TileMagic_ClearBorder: failled as border width (" +
						IntToString(nWidth) + ") was invalid.";
		SJ_Debug(SJ_DEBUG_PREFIX_ERROR + sDebug, TRUE);
		return;
	}
...
void SJ_TileMagic_CoverLargeArea(object oArea, int nTile, float fZ = 0.0, int bNormalise = TRUE) {
	...
	AssignCommand(OBJECT_SELF, SJ_TileMagic_CoverRange(oArea, nTile, nCol, 0, ( nCols -
																			   nCol ), nRow, fZ, bNormalise));
	AssignCommand(OBJECT_SELF, SJ_TileMagic_CoverRange(oArea, nTile, 0, nRow, nCol, ( nRows -
																					 nRow ), fZ, bNormalise));
	AssignCommand(OBJECT_SELF, SJ_TileMagic_CoverRange(oArea, nTile, nCol, nRow, ( nCols -
																				  nCol ), ( nRows -
																						   nRow ), fZ,
			bNormalise));
}
...
void SJ_TileMagic_ClearRange(object oArea, int nColumn = 0, int nRow = 0, int nColumns = 1, int nRows = 1) {
	...
	if ( nColumns < 1 || nRows < 1 ) {
		// log an error
		string sDebug = "SJ_TileMagic_ClearRange: failed as the number of columns (" +
						IntToString(nColumns) + ") or rows (" + IntToString(nRows) + ") was invalid.";
		SJ_Debug(SJ_DEBUG_PREFIX_ERROR + sDebug, TRUE);
		return;
	}
...
void SJ_TileMagic_ClearSpot(object oArea, float fX, float fY) {
	...
	if ( GetIsObjectValid(oTile) ) {
		...
		// log an error
		string sDebug =  "SJ_TileMagic_ClearSpot: failed as target (" +
						SJ_TileMagic_TileToString(oArea, sTag) + ") did not exist.";
		SJ_Debug(SJ_DEBUG_PREFIX_ERROR + sDebug, TRUE);
	}
}
...
void SJ_TileMagic_ClearTile(object oArea, int nColumn = 0, int nRow = 0) {
	...
	if ( GetIsObjectValid(oTile) ) {
		...
		// log an error
		string sDebug =  "SJ_TileMagic_ClearTile: failed as target (" +
						SJ_TileMagic_TileToString(oArea, sTag) + ") was not a valid object.";
		SJ_Debug(SJ_DEBUG_PREFIX_ERROR + sDebug, TRUE);
	}
}
...
void SJ_TileMagic_CoverBorder(object oArea, int nTile, int nWidth = 1, float fZ = 0.0, int bNormalise = TRUE,
	...
	// sanity check
	if ( nWidth < 1 ) {
		string sDebug = "SJ_TileMagic_CoverBorder: failled as border width (" +
						IntToString(nWidth) + ") was invalid.";
		SJ_Debug(SJ_DEBUG_PREFIX_ERROR + sDebug, TRUE);
		return;
	}
		...
		for ( nColumn = 0; nColumn < nColumns; nColumn++ ) {
			...
			if ( sStyle != "" && sStyle != " " && sStyle != SJ_TILEMAGIC_STYLE_NONE ) {
				...
				if ( GetLocalString(oPalette, sStyle) == "" ) {
					...
					string sDebug = "SJ_TileMagic_CoverMatrix: style (" +
									sStyle + ") did not exist in palette.";
					SJ_Debug(SJ_DEBUG_PREFIX_ERROR + sDebug, TRUE);
					continue;
				}
...
void SJ_TileMagic_CoverRange(object oArea, int nTile, int nColumn = 0, int nRow = 0, int nColumns = 1, int nRows = 1, float fZ = 0.0, int bNormalise = TRUE,
	...
	if ( nColumns < 1 || nRows < 1 ) {
		// log an error
		string sDebug = "SJ_TileMagic_CoverRange: failed as the number of columns (" +
						IntToString(nColumns) + ") or rows (" + IntToString(nRows) + ") was invalid.";
		SJ_Debug(SJ_DEBUG_PREFIX_ERROR + sDebug, TRUE);
		return;
	}
...
void SJ_TileMagic_CoverSpot(object oArea, int nTile, float fX, float fY, float fZ = 0.0, int bNormalise = TRUE,
	...
	if ( SJ_GetIsSpotValid(oArea, fX, fY) ) {
		...
		// log an error
		string sDebug =  "SJ_TileMagic_CoverSpot: failed as target (" +
						SJ_TileMagic_TileToString(oArea, sTag, nTile) + ") was outwith area.";
		SJ_Debug(SJ_DEBUG_PREFIX_ERROR + sDebug, TRUE);
	}
}
...
void SJ_TileMagic_CoverTile(object oArea, int nTile, int nColumn, int nRow, float fZ = 0.0, int bNormalise = TRUE,
	...
	if ( SJ_GetIsTileValid(oArea, nColumn, nRow) ) {
		...
		// log an error
		string sDebug =  "SJ_TileMagic_CoverTile: failed as target (" +
						SJ_TileMagic_TileToString(oArea, sTag, nTile) + ") was outwith area.";
		SJ_Debug(SJ_DEBUG_PREFIX_ERROR + sDebug, TRUE);
	}
}
...
object SJ_TileMagic_CreateMatrix(string sName, int bUse2DA = FALSE) {
	...
	if ( GetStringLength(sName) > 16 ) {
		// log an error
		string sDebug = "SJ_TileMagic_CreateMatrix: resource name (" +
						sName + ") exceeded 16 characters and is invalid.";
		SJ_Debug(SJ_DEBUG_PREFIX_ERROR + sDebug, TRUE);
...
object SJ_TileMagic_CreatePalette(string sName, int bUse2DA = FALSE) {
	...
	if ( GetStringLength(sName) > 16 ) {
		// log an error
		string sDebug = "SJ_TileMagic_CreatePalette: resource name (" +
						sName + ") exceeded 16 characters and is invalid.";
		SJ_Debug(SJ_DEBUG_PREFIX_ERROR + sDebug, TRUE);
		return OBJECT_INVALID;
	}
