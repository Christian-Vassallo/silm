...
void DoControlFunction(int iFaction, object oUser) {
	...
	switch ( iFaction ) {
		...
		case 61: AssignCommand(oTarget, ClearAllActions()); AssignCommand(oTarget, ActionMoveAwayFromObject(
					oUser, TRUE)); break;
		case 62: AssignCommand(oTarget, ClearAllActions()); AssignCommand(oTarget, ActionForceMoveToObject(
					oUser, TRUE, 2.0f, 30.0f)); break;
		...
		case 65: oChange = GetFirstObjectInArea(oArea);
			while ( GetIsObjectValid(oChange) ) {
				if ( GetObjectType(oChange) == OBJECT_TYPE_CREATURE && !GetIsPC(oChange) ) {
					AssignCommand(oChange, ClearAllActions()); AssignCommand(oChange,
						ActionMoveAwayFromObject(oUser, TRUE));
				}
				oChange = GetNextObjectInArea(oArea);
			}
			break;
		case 66: oChange = GetFirstObjectInArea(oArea);
			while ( GetIsObjectValid(oChange) ) {
				if ( GetObjectType(oChange) == OBJECT_TYPE_CREATURE && !GetIsPC(oChange) ) {
					AssignCommand(oChange, ClearAllActions()); AssignCommand(oChange, ActionForceMoveToObject(
							oUser, TRUE, 2.0f, 30.0f));
				}
				oChange = GetNextObjectInArea(oArea);
			}
			break;
		...
		case 75: AssignCommand(oTarget, SetIsDestroyable(FALSE, TRUE, TRUE));
			DelayCommand(0.1, AssignCommand(oTarget, ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectDeath(),
						oTarget))); break;
...
void CreateSetting(object oUser) {
	...
	DelayCommand(0.5f, AssignCommand(oSetting, ActionSpeakString(GetLocalString(oUser, "EffectSetting") +
				" is currently set at " +
				FloatToString(GetLocalFloat(oUser, GetLocalString(oUser, "EffectSetting"))))));
...
void RollDemBones(object oUser, int iBroadcast = 0, int iMod = 0, string sAbility = "", int iNum = 1,
				  int iSide = 20) {
	string sString = "";
	int iRoll = 0;
	int iTotal = 0;

	int iOverride = GetLocalInt(oUser, "d_o");
	if ( iOverride > 20 )
		iOverride = 20;
	int iOverrideMod = GetLocalInt(oUser, "d_om");

	SetLocalInt(oUser, "d_o", 0);
	SetLocalInt(oUser, "d_om", 0);

	iMod += iOverrideMod;

	//Build the string
	sString = sAbility + "Werfe " + IntToString(iNum) + "w" + IntToString(iSide) + ": ";
	while ( iNum > 1 ) {
		iRoll = iOverride > 0 ? iOverride : Random(iSide) + 1;
		iTotal = iTotal + iRoll;
	...
	if ( iMod ) {
		iTotal = iTotal + iMod;
		sString = sString + " + Modifikator: " + IntToString(iMod);
	}
	...
	switch ( GetLocalInt(oUser, "dmfi_univ_int") ) {
		...
		case 92: AssignCommand(oUser, ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectVisualEffect(
						VFX_DUR_BARD_SONG), oUser, 6.0f)); break;
...
void DoDiceBagFunction(int iDice, object oUser, int iDMOverride = 0) {
	...
	switch ( iDice ) {
		case 71: iNum = 1; iSide = 20; sAbility = "Stärke Check, "; iMod = GetAbilityModifier(
				ABILITY_STRENGTH, oUser); break;
		case 72: iNum = 1; iSide = 20; sAbility = "Geschicklichkeit Check, "; iMod = GetAbilityModifier(
				ABILITY_DEXTERITY, oUser); break;
		case 73: iNum = 1; iSide = 20; sAbility = "Konstitution Check, "; iMod = GetAbilityModifier(
				ABILITY_CONSTITUTION, oUser); break;
		case 74: iNum = 1; iSide = 20; sAbility = "Intelligenz Check, "; iMod = GetAbilityModifier(
				ABILITY_INTELLIGENCE, oUser); break;
		case 75: iNum = 1; iSide = 20; sAbility = "Weisheit Check, "; iMod = GetAbilityModifier(
				ABILITY_WISDOM, oUser); break;
		case 76: iNum = 1; iSide = 20; sAbility = "Charisma Check, "; iMod = GetAbilityModifier(
				ABILITY_CHARISMA, oUser); break;
		case 77: iNum = 1; iSide = 20; sAbility = "Zähigkeitsrettungswurf, "; iMod = GetFortitudeSavingThrow(
				oUser); break;
		case 78: iNum = 1; iSide = 20; sAbility = "Reflexrettungswurf, "; iMod = GetReflexSavingThrow(oUser);
			break;
		case 79: iNum = 1; iSide = 20; sAbility = "Willensrettungswurf, "; iMod = GetWillSavingThrow(oUser);
			break;
		case 80: iNum = 1; iSide = 20; sAbility = "Schlösser öffnen Check, "; iMod = GetSkillRank(
				SKILL_OPEN_LOCK, oUser); break;
		case 81: iNum = 1; iSide = 20; sAbility = "Gefühl für Tiere Check, "; iMod = GetSkillRank(
				SKILL_ANIMAL_EMPATHY, oUser); break;
		case 82: iNum = 1; iSide = 20; sAbility = "Konzentration Check, "; iMod = GetSkillRank(
				SKILL_CONCENTRATION, oUser); break;
		case 83: iNum = 1; iSide = 20; sAbility = "Fallen entschärfen Check, "; iMod = GetSkillRank(
				SKILL_DISABLE_TRAP, oUser); break;
		case 84: iNum = 1; iSide = 20; sAbility = "Disziplin Check, "; iMod = GetSkillRank(SKILL_DISCIPLINE,
																				  oUser); break;
		case 85: iNum = 1; iSide = 20; sAbility = "Heilen Check, "; iMod = GetSkillRank(SKILL_HEAL, oUser);
			break;
		case 86: iNum = 1; iSide = 20; sAbility = "Verstecken Check, "; iMod = GetSkillRank(SKILL_HIDE, oUser);
			break;
		case 87: iNum = 1; iSide = 20; sAbility = "Lauschen Check, "; iMod = GetSkillRank(SKILL_LISTEN, oUser);
			break;
		case 88: iNum = 1; iSide = 20; sAbility = "Sagenkunde Check, "; iMod = GetSkillRank(SKILL_LORE, oUser);
			break;
		case 89: iNum = 1; iSide = 20; sAbility = "Leise Bewegen Check, "; iMod = GetSkillRank(
				SKILL_MOVE_SILENTLY, oUser); break;
		case 90: iNum = 1; iSide = 20; sAbility = "Mag. Geg. benutzen Check, "; iMod = GetSkillRank(
				SKILL_USE_MAGIC_DEVICE, oUser); break;
		case 91: iNum = 1; iSide = 20; sAbility = "Parieren Check, "; iMod = GetSkillRank(SKILL_PARRY, oUser);
			break;
		case 92: iNum = 1; iSide = 20; sAbility = "Auftreten Check, "; iMod = GetSkillRank(SKILL_PERFORM,
																				  oUser); break;
		case 93: iNum = 1; iSide = 20; sAbility = "Überzeugen Check, "; iMod = GetSkillRank(SKILL_PERSUADE,
																				   oUser); break;
		case 94: iNum = 1; iSide = 20; sAbility = "Taschendiebstahl Check, "; iMod = GetSkillRank(
				SKILL_PICK_POCKET, oUser); break;
		case 95: iNum = 1; iSide = 20; sAbility = "Suchen Check, "; iMod = GetSkillRank(SKILL_SEARCH, oUser);
			break;
		case 96: iNum = 1; iSide = 20; sAbility = "Fallen stellen Check, "; iMod = GetSkillRank(
				SKILL_SET_TRAP, oUser); break;
		case 97: iNum = 1; iSide = 20; sAbility = "Zauberkunde Check, "; iMod = GetSkillRank(SKILL_SPELLCRAFT,
																					oUser); break;
		case 98: iNum = 1; iSide = 20; sAbility = "Entdecken Check, "; iMod = GetSkillRank(SKILL_SPOT, oUser);
			break;
		case 99: iNum = 1; iSide = 20; sAbility = "Provozieren Check, "; iMod = GetSkillRank(SKILL_TAUNT,
																					oUser); break;
		case 101: SetLocalInt(oUser, "dmfi_dicebag", 2); SetCustomToken(20681, "Local");
			FloatingTextStringOnCreature("Übertragunsmodus auf Lokal gesetzt", oUser, FALSE); return; break;
		case 102: SetLocalInt(oUser, "dmfi_dicebag", 1); SetCustomToken(20681, "Global");
			FloatingTextStringOnCreature("Übertragunsmodus auf Global gesetzt", oUser, FALSE); return; break;
		case 103: SetLocalInt(oUser, "dmfi_dicebag", 0); SetCustomToken(20681, "Private");
			FloatingTextStringOnCreature("Übertragunsmodus auf Privat gesetzt", oUser, FALSE); return; break;
		...
		case 111: iNum = 1; iSide = 20; sAbility = "Schätzen Check, "; iMod = GetSkillRank(SKILL_APPRAISE,
																				  oUser); break;
		case 112: iNum = 1; iSide = 20; sAbility = "Turnen Check, "; iMod = GetSkillRank(SKILL_TUMBLE, oUser);
			break;
		case 113: iNum = 1; iSide = 20; sAbility = "Bluffen Check, "; iMod = GetSkillRank(SKILL_BLUFF, oUser);
			break;
		case 114: iNum = 1; iSide = 20; sAbility = "Einschuechtern Check, "; iMod = GetSkillRank(
				SKILL_INTIMIDATE, oUser); break;
		case 115: iNum = 1; iSide = 20; sAbility = "Falle herstellen Check, "; iMod = GetSkillRank(
				SKILL_CRAFT_TRAP, oUser); break;
		default: iNum = iRight;
			switch ( iLeft ) {
	...
	switch ( iLeft ) {
		...
		case 6: //All PCs/NPCs in the area
			...
			while ( GetIsObjectValid(oRoll) ) {
				if ( ( GetIsPC(oTarget) && GetIsPC(oRoll) )
					|| ( !GetIsPC(oTarget) && !GetIsPC(oRoll) && GetObjectType(oRoll) == OBJECT_TYPE_CREATURE ) )
		...
		case 10:
			switch ( iDice ) {
				case 101: SetLocalInt(oUser, "dmfi_dicebag", 2); SetCustomToken(20681, "Local");
					FloatingTextStringOnCreature("Übertragunsmodus auf Lokal gesetzt", oUser, FALSE); return;

					break;
				case 102: SetLocalInt(oUser, "dmfi_dicebag", 1); SetCustomToken(20681, "Global");
					FloatingTextStringOnCreature("Übertragunsmodus auf Global gesetzt", oUser, FALSE); return;

					break;
				case 103: SetLocalInt(oUser, "dmfi_dicebag", 0); SetCustomToken(20681, "Private");
					FloatingTextStringOnCreature("Übertragunsmodus auf Privat gesetzt", oUser, FALSE); return;

					break;
				case 104: SetLocalInt(oUser, "dmfi_dicebag", 3); SetCustomToken(20681, "DM Only");
					FloatingTextStringOnCreature("Übertragunsmodus auf nur SL gesetzt", oUser, FALSE); return;
...
void DoSoundFunction(int iSound, object oUser) {
	...
	switch ( iSound ) {
		case 11: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_an_batsflap1"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 12: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_an_bugsscary1"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 13: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_pl_crptvoice1"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 14: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_an_orcgrunt1"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 15: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_cv_minepick2"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 16: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_an_ratssqeak1"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 17: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_na_rockfallg1"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 18: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_na_rockfalgl2"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 19: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_wt_gustcavrn1"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 21: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_cv_belltower3"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 22: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_cv_claybreak3"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 23: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_cv_glasbreak2"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 24: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_cv_gongring3"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 25: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_pl_marketgrp4"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 26: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("al_cv_millwheel1"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 27: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_cv_sawing1"))); DelayCommand(20.0f,
				DestroyObject(oTarget));
			break;
		case 28: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_cv_bellwind1"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 29: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("al_cv_smithhamr2"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 31: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("al_na_firelarge1"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 32: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("al_na_lavapillr1"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 33: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("al_na_lavafire1"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 34: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("al_na_firelarge2"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 35: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_na_surf2"))); DelayCommand(20.0f, DestroyObject(
					oTarget));
			break;
		case 36: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("al_na_drips1"))); DelayCommand(20.0f, DestroyObject(
					oTarget));
			break;
		case 37: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_na_waterlap1"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 38: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("al_na_stream4"))); DelayCommand(20.0f,
				DestroyObject(oTarget));
			break;
		case 39: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("al_na_waterfall2"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 41: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_an_crynight3"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 42: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_na_bushmove1"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 43: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_an_birdsflap2"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 44: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_na_grassmove3"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 45: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_an_hawk1"))); DelayCommand(20.0f, DestroyObject(
					oTarget));
			break;
		case 46: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_na_leafmove3"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 47: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_an_gulls2"))); DelayCommand(20.0f, DestroyObject(
					oTarget));
			break;
		case 48: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_an_songbirds1"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 49: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("al_an_toads1"))); DelayCommand(20.0f, DestroyObject(
					oTarget));
			break;
		case 51: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("al_mg_beaker1"))); DelayCommand(20.0f,
				DestroyObject(oTarget));
			break;
		case 52: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("al_mg_cauldron1"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 53: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("al_mg_chntmagic1"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 54: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("al_mg_crystalev1"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 55: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("al_mg_crystalic1"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 56: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("al_mg_portal1"))); DelayCommand(20.0f,
				DestroyObject(oTarget));
			break;
		case 57: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_mg_telepin1"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 58: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_mg_telepout1"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 59: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_mg_frstmagic1"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 61: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_pl_tavclap1"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 62: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_pl_battlegrp7"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 63: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_pl_laughincf2"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 64: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_pl_comtntgrp3"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 65: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_pl_chantingm2"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 66: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_pl_cryingf2"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 67: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_pl_laughingf3"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 68: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_pl_chantingf2"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 69: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_pl_wailingm6"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 71: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_pl_evilchantm"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 72: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_an_crows2"))); DelayCommand(20.0f, DestroyObject(
					oTarget));
			break;
		case 73: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_pl_wailingcf1"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 74: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_pl_crptvoice2"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 75: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_pl_lafspook2"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 76: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_an_owlhoot1"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 77: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_an_wolfhowl1"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 78: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_pl_screamf3"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 79: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_pl_zombiem3"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 81: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_wt_gustsoft1"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 82: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_wt_thundercl3"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 83: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_wt_thunderds4"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
		case 84: oTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", lLocation); DelayCommand(
				fDelay, AssignCommand(oTarget, PlaySound("as_wt_gusforst1"))); DelayCommand(20.0f,
				DestroyObject(
					oTarget)); break;
			/*case 85: weather_update(WEATHER_CLEAR); break;
			 * case 86: weather_update(WEATHER_RAIN); break;
			 * case 87: weather_update(WEATHER_SNOW); break;
			 * case 88: weather_update(); break;
			 * case 89: weather_update(); break;*/
			//Settings
		case 91:
...
void DoAfflictFunction(int iAfflict, object oUser) {
	...
	if ( !( GetObjectType(oTarget) == OBJECT_TYPE_CREATURE )
		|| GetIsDM(oTarget) ) {
		FloatingTextStringOnCreature("Du musst eine gültige Kreatur als Ziel haben!", oUser, FALSE);
		return;
	}
	switch ( iAfflict ) {
		case 11: ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectDamage(1, DAMAGE_TYPE_MAGICAL,
					DAMAGE_POWER_NORMAL), oTarget);
			ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_COM_BLOOD_SPARK_SMALL), oTarget);
			break;
		case 12: ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectDamage(5, DAMAGE_TYPE_MAGICAL,
					DAMAGE_POWER_NORMAL), oTarget); break;
			ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_COM_BLOOD_LRG_RED), oTarget);
			break;
		case 13: ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectDamage(10, DAMAGE_TYPE_MAGICAL,
					DAMAGE_POWER_NORMAL), oTarget); break;
			ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_COM_BLOOD_LRG_RED), oTarget);
			break;
		case 14: ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectDamage(25, DAMAGE_TYPE_MAGICAL,
					DAMAGE_POWER_NORMAL), oTarget); break;
			ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_COM_CHUNK_RED_SMALL), oTarget);
			break;
		case 15: ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectDamage(50, DAMAGE_TYPE_MAGICAL,
					DAMAGE_POWER_NORMAL), oTarget); break;
			ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_COM_CHUNK_RED_SMALL), oTarget);
			break;
		case 16: ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectDamage(GetCurrentHitPoints(oTarget) /
					4, DAMAGE_TYPE_MAGICAL, DAMAGE_POWER_NORMAL), oTarget); break;
			ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_COM_BLOOD_LRG_RED), oTarget);
			break;
		case 17: ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectDamage(GetCurrentHitPoints(oTarget) /
					2, DAMAGE_TYPE_MAGICAL, DAMAGE_POWER_NORMAL), oTarget); break;
			ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_COM_BLOOD_LRG_RED), oTarget);
			break;
		case 18: ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectDamage(GetCurrentHitPoints(oTarget) *
					3 / 4, DAMAGE_TYPE_MAGICAL, DAMAGE_POWER_NORMAL), oTarget); break;
			ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_COM_CHUNK_RED_SMALL), oTarget);
			break;
		case 19: ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectDamage(GetCurrentHitPoints(oTarget) -
					1, DAMAGE_TYPE_MAGICAL, DAMAGE_POWER_NORMAL), oTarget); break;
			ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_COM_CHUNK_RED_SMALL), oTarget);
			break;
		case 21: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectDisease(DISEASE_FILTH_FEVER), oTarget);
			break;
		case 22: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectDisease(DISEASE_MINDFIRE), oTarget);
			break;
		case 23: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectDisease(DISEASE_DREAD_BLISTERS), oTarget);
			break;
		case 24: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectDisease(DISEASE_SHAKES), oTarget); break;
		case 25: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectDisease(DISEASE_VERMIN_MADNESS), oTarget);
			break;
		case 26: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectDisease(DISEASE_DEVIL_CHILLS), oTarget);
			break;
		case 27: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectDisease(DISEASE_SLIMY_DOOM), oTarget);
			break;
		case 28: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectDisease(DISEASE_RED_ACHE), oTarget);
			break;
		case 29: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectDisease(DISEASE_ZOMBIE_CREEP), oTarget);
			break;
		case 31: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectDisease(DISEASE_BLINDING_SICKNESS),
				oTarget); break;
		case 32: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectDisease(DISEASE_CACKLE_FEVER), oTarget);
			break;
		case 33: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectDisease(DISEASE_BURROW_MAGGOTS), oTarget);
			break;
		case 34: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectDisease(DISEASE_RED_SLAAD_EGGS), oTarget);
			break;
		case 35: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectDisease(DISEASE_DEMON_FEVER), oTarget);
			break;
		case 36: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectDisease(DISEASE_GHOUL_ROT), oTarget);
			break;
		case 37: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectDisease(DISEASE_MUMMY_ROT), oTarget);
			break;
		case 38: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectDisease(DISEASE_SOLDIER_SHAKES), oTarget);
			break;
		case 39: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectDisease(DISEASE_SOLDIER_SHAKES), oTarget);
			break;
		case 41: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectPoison(POISON_TINY_SPIDER_VENOM), oTarget);
			break;
		case 42: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectPoison(POISON_ARANEA_VENOM), oTarget);
			break;
		case 43: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectPoison(POISON_MEDIUM_SPIDER_VENOM),
				oTarget); break;
		case 44: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectPoison(POISON_CARRION_CRAWLER_BRAIN_JUICE),
				oTarget); break;
		case 45: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectPoison(POISON_OIL_OF_TAGGIT), oTarget);
			break;
		case 46: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectPoison(POISON_ARSENIC), oTarget); break;
		case 47: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectPoison(POISON_GREENBLOOD_OIL), oTarget);
			break;
		case 48: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectPoison(POISON_NITHARIT), oTarget); break;
		case 49: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectPoison(POISON_PHASE_SPIDER_VENOM),
				oTarget); break;
		case 51: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectPoison(POISON_LICH_DUST), oTarget); break;
		case 52: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectPoison(POISON_SHADOW_ESSENCE), oTarget);
			break;
		case 53: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectPoison(POISON_LARGE_SPIDER_VENOM),
				oTarget); break;
		case 54: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectPoison(POISON_PURPLE_WORM_POISON),
				oTarget); break;
		case 55: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectPoison(POISON_IRON_GOLEM), oTarget);
			break;
		case 56: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectPoison(POISON_PIT_FIEND_ICHOR), oTarget);
			break;
		case 57: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectPoison(POISON_WYVERN_POISON), oTarget);
			break;
		case 58: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectPoison(POISON_BLACK_LOTUS_EXTRACT),
				oTarget); break;
		case 59: ApplyEffectToObject(DURATION_TYPE_PERMANENT, EffectPoison(POISON_GARGANTUAN_SPIDER_VENOM),
				oTarget); break;
		case 61: ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectBlindness(), oTarget, fDuration);
			ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectVisualEffect(VFX_DUR_BLIND), oTarget,
				fDuration); break;
		case 62: ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectCurse(4, 4, 4, 4, 4, 4), oTarget,
				fDuration); break;
		case 63: ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectFrightened(), oTarget, fDuration);
			ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectVisualEffect(VFX_DUR_MIND_AFFECTING_FEAR),
				oTarget, fDuration); break;
		...
		case 69: ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectDamage(GetCurrentHitPoints(oTarget) -
					1, DAMAGE_TYPE_MAGICAL, DAMAGE_POWER_NORMAL), oTarget);
		...
		case 71: FloatingTextStringOnCreature("Du hast 50 EP verloren!", oTarget);
			SetXP(oTarget, GetXP(oTarget) - 50); break;
		case 72: FloatingTextStringOnCreature("Du hast 100 EP verloren!", oTarget);
			SetXP(oTarget, GetXP(oTarget) - 100); break;
		case 73: FloatingTextStringOnCreature("Du hast 250 EP verloren!", oTarget);
			SetXP(oTarget, GetXP(oTarget) - 250); break;
		case 74: FloatingTextStringOnCreature("Du hast 500 EP verloren!", oTarget);
			SetXP(oTarget, GetXP(oTarget) - 500); break;
		case 75: FloatingTextStringOnCreature("Du hast 1000 EP verloren!", oTarget);
			SetXP(oTarget, GetXP(oTarget) - 1000); break;
		case 76: FloatingTextStringOnCreature("Du hast 2000 EP verloren!", oTarget);
			SetXP(oTarget, GetXP(oTarget) - 2000); break;
		case 77: FloatingTextStringOnCreature("Du hast " +
				IntToString(GetXP(oTarget) -
					( ( GetHitDice(oTarget) * ( GetHitDice(oTarget) - 1 ) ) / 2 * 1000 )) +
				" EP verloren!", oTarget);
			SetXP(oTarget, ( ( GetHitDice(oTarget) * ( GetHitDice(oTarget) - 1 ) ) / 2 * 1000 )); break;
		case 78: FloatingTextStringOnCreature("Du hast einen Level verloren!", oTarget);
			SetXP(oTarget, ( ( GetHitDice(oTarget) * ( GetHitDice(oTarget) - 1 ) ) / 2 * 1000 ) -
				( ( ( GetHitDice(oTarget) - 1 ) * 1000 ) / 2 )); break;
		case 79: FloatingTextStringOnCreature("Du hast alle EP verloren!", oTarget);
...
void DoMusicFunction(int iMusic, object oUser) {
	...
	switch ( iMusic ) {
		...
		case 2: MusicBackgroundStop(GetArea(oUser)); DelayCommand(1.0, MusicBackgroundStop(GetArea(oUser)));
			return; break;
...
void DoXPFunction(int iXP, object oUser) {
	...
	if ( !GetIsObjectValid(oTarget)
		|| !GetIsPC(oTarget) ) {
		FloatingTextStringOnCreature("Du hast keinen PC als Ziel", oUser, FALSE);
		return;
	}
	switch ( iXP ) {
		...
		case 61:   iHD = GetHitDice(oTarget);
			SendMessageToPC(oUser, GetName(oTarget) +
				" has received " +
				IntToString(GetLocalInt(oPartyMember, "dmfi_XPGiven")) + " XP this session.");
			SendMessageToPC(oUser, GetName(oTarget) +
				" currently has " + IntToString(GetXP(oTarget)) + " total XP.");
			SendMessageToPC(oUser, GetName(oTarget) +
				" currently needs " +
				IntToString(( ( iHD * ( iHD + 1 ) ) / 2 * 1000 ) - GetXP(oTarget)) + " to level.");
			SendMessageToPC(oUser, GetName(oTarget) + " has " + IntToString(GetGold(oTarget)) + " gp.");
			SendMessageToPC(oUser, GetName(oTarget) +
				" has items totaling " + IntToString(DMFI_GetNetWorth(oTarget)) + " in gp value.");
			return; break;
		case 62:   oPartyMember = GetFirstFactionMember(oTarget, TRUE);
			while ( GetIsObjectValid(oPartyMember) == TRUE ) {
				...
				SendMessageToPC(oUser, GetName(oPartyMember) +
					" has " + IntToString(GetXP(oPartyMember)) + " XP total.");
		...
		case 64:   oPartyMember = GetFirstFactionMember(oTarget, TRUE);
			while ( GetIsObjectValid(oPartyMember) == TRUE ) {
				...
				SendMessageToPC(oUser, GetName(oPartyMember) +
					" is level " +
					IntToString(GetHitDice(oPartyMember)) +
					" and needs " +
					IntToString(( ( iHD * ( iHD + 1 ) ) / 2 * 1000 ) - GetXP(oPartyMember)) +
					" XP to level up.");
				oPartyMember = GetNextFactionMember(oTarget, TRUE);
			}
			return; break;
	...
	if ( iParty && iReward ) {
		...
		while ( GetIsObjectValid(oPartyMember) == TRUE ) {
			...
			SendMessageToAllDMs(GetName(oPartyMember) +
				" was granted a " +
				GetLocalString(oUser, "BonusType") + " XP reward of " + IntToString(iReward) + ".");
			oPartyMember = GetNextFactionMember(oTarget, TRUE);
		}
		...
		SendMessageToAllDMs(GetName(oTarget) +
			" was granted a " +
			GetLocalString(oUser, "BonusType") + " experience reward of " + IntToString(iReward) + ".");
	}
	return;
...
void FXWand_Firestorm(object oDM) {
	...
	AssignCommand(oDM, ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_METEOR_SWARM),
			lDMLoc));
	AssignCommand(oDM, DelayCommand(1.0, ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(
					VFX_FNF_SCREEN_SHAKE), lDMLoc)));
...
void FXWand_Earthquake(object oDM) {
	...
	AssignCommand(oDM, ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_SCREEN_SHAKE),
			lDMLoc));
	AssignCommand(oDM, DelayCommand(2.8, ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(
					VFX_FNF_SCREEN_BUMP), lDMLoc)));
	AssignCommand(oDM, DelayCommand(3.0, ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(
					VFX_FNF_SCREEN_SHAKE), lDMLoc)));
	AssignCommand(oDM, DelayCommand(4.5, ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(
					VFX_FNF_SCREEN_BUMP), lDMLoc)));
	AssignCommand(oDM, DelayCommand(5.8, ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(
					VFX_FNF_SCREEN_BUMP), lDMLoc)));
...
void FXWand_Lightning(object oDM) {
	...
	AssignCommand(oDM, ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_IMP_LIGHTNING_M),
			lDMLoc));
	...
	for ( nCount = 0; nCount < 5; nCount++ ) {
		...
		AssignCommand(oDM, ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(
...
void CreateEffects(int iEffect, location lEffect, object oUser) {
	...
	switch ( iEffect ) {
		...
		case 11: ApplyEffectAtLocation(DURATION_TYPE_TEMPORARY, EffectVisualEffect(VFX_DUR_TENTACLE), lEffect,
				fDuration); break;
		case 12: ApplyEffectAtLocation(DURATION_TYPE_TEMPORARY, EffectVisualEffect(VFX_DUR_WEB_MASS), lEffect,
				fDuration); break;
		case 13: ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_GAS_EXPLOSION_MIND),
				lEffect); break;
		case 14: ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_LOS_HOLY_30),
				lEffect, fDuration); break;
		case 15: ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_LOS_EVIL_30),
				lEffect, fDuration); break;
		case 16: ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_SMOKE_PUFF), lEffect,
				fDuration); break;
		case 17: ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_HOWL_WAR_CRY),
				lEffect, fDuration); break;
		case 18: ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_DISPEL_DISJUNCTION),
				lEffect, fDuration); break;
		case 19: ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_IMPLOSION), lEffect,
				fDuration); break;
			//Magical Status Effects (must have a target)
		case 21: ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectVisualEffect(VFX_DUR_PROT_BARKSKIN),
				oTarget, fDuration); break;
		case 22: ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectVisualEffect(
					VFX_DUR_PROT_GREATER_STONESKIN), oTarget, fDuration); break;
		case 23: ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectVisualEffect(VFX_DUR_ENTANGLE), oTarget,
				fDuration); break;
		case 24: ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectVisualEffect(VFX_DUR_ETHEREAL_VISAGE),
				oTarget, fDuration); break;
		case 25: ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectVisualEffect(VFX_DUR_GHOSTLY_VISAGE),
				oTarget, fDuration); break;
		case 26: ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectVisualEffect(VFX_DUR_INVISIBILITY),
				oTarget, fDuration); break;
		case 27: ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectVisualEffect(VFX_DUR_BARD_SONG), oTarget,
				fDuration); break;
		case 28: ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectVisualEffect(
					VFX_DUR_GLOBE_INVULNERABILITY), oTarget, fDuration); break;
		case 29: ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectVisualEffect(VFX_DUR_BLIND), oTarget,
				fDuration); break;
			//Magical Burst Effects
		case 31: ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_FIREBALL), lEffect);
			break;
		case 32: ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_FIRESTORM), lEffect);
			break;
		case 33: ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_HORRID_WILTING),
				lEffect); break;
		case 34: ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_METEOR_SWARM),
				lEffect); break;
		case 35: ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_SCREEN_BUMP),
				lEffect); break;
		case 36: ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_SOUND_BURST),
				lEffect); break;
		case 37: ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_STRIKE_HOLY),
				lEffect); break;
		case 38: ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_WAIL_O_BANSHEES),
				lEffect); break;
		case 39: ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_WEIRD), lEffect);
			break;
			//Lighting Effects
		case 41: ApplyEffectAtLocation(DURATION_TYPE_TEMPORARY, EffectVisualEffect(VFX_DUR_BLACKOUT), lEffect,
				fDuration); break;
		case 42: ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectVisualEffect(VFX_DUR_ANTI_LIGHT_10),
				oTarget, fDuration); break;
		case 43: ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectVisualEffect(VFX_DUR_LIGHT_BLUE_20),
				oTarget, fDuration); break;
		case 44: ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectVisualEffect(VFX_DUR_LIGHT_GREY_20),
				oTarget, fDuration); break;
		case 45: ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectVisualEffect(VFX_DUR_LIGHT_ORANGE_20),
				oTarget, fDuration); break;
		case 46: ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectVisualEffect(VFX_DUR_LIGHT_PURPLE_20),
				oTarget, fDuration); break;
		case 47: ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectVisualEffect(VFX_DUR_LIGHT_RED_20),
				oTarget, fDuration); break;
		case 48: ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectVisualEffect(VFX_DUR_LIGHT_WHITE_20),
				oTarget, fDuration); break;
		case 49: ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectVisualEffect(VFX_DUR_LIGHT_YELLOW_20),
				oTarget, fDuration); break;
			//Beam Effects
		case 51: ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectBeam(VFX_BEAM_COLD, oUser,
					BODY_NODE_CHEST, FALSE), oTarget, fBeamDuration); break;
		case 52: ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectBeam(VFX_BEAM_EVIL, oUser,
					BODY_NODE_CHEST, FALSE), oTarget, fBeamDuration); break;
		case 53: ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectBeam(VFX_BEAM_FIRE, oUser,
					BODY_NODE_CHEST, FALSE), oTarget, fBeamDuration); break;
		case 54: ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectBeam(VFX_BEAM_FIRE_LASH, oUser,
					BODY_NODE_CHEST, FALSE), oTarget, fBeamDuration); break;
		case 55: ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectBeam(VFX_BEAM_HOLY, oUser,
					BODY_NODE_CHEST, FALSE), oTarget, fBeamDuration); break;
		case 56: ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectBeam(VFX_BEAM_LIGHTNING, oUser,
					BODY_NODE_CHEST, FALSE), oTarget, fBeamDuration); break;
		case 57: ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectBeam(VFX_BEAM_MIND, oUser,
					BODY_NODE_CHEST, FALSE), oTarget, fBeamDuration); break;
		case 58: ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectBeam(VFX_BEAM_ODD, oUser, BODY_NODE_CHEST,
					FALSE), oTarget, fBeamDuration); break;
		case 59: ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectBeam(VFX_BEAM_COLD, oUser,
					BODY_NODE_CHEST, FALSE), oTarget, fBeamDuration);
			ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectBeam(VFX_BEAM_EVIL, oUser, BODY_NODE_CHEST,
					FALSE), oTarget, fBeamDuration);
			ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectBeam(VFX_BEAM_FIRE, oUser, BODY_NODE_CHEST,
					FALSE), oTarget, fBeamDuration);
			ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectBeam(VFX_BEAM_FIRE_LASH, oUser,
					BODY_NODE_CHEST, FALSE), oTarget, fBeamDuration);
			ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectBeam(VFX_BEAM_HOLY, oUser, BODY_NODE_CHEST,
					FALSE), oTarget, fBeamDuration);
			ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectBeam(VFX_BEAM_LIGHTNING, oUser,
					BODY_NODE_CHEST, FALSE), oTarget, fBeamDuration);
			ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectBeam(VFX_BEAM_MIND, oUser, BODY_NODE_CHEST,
					FALSE), oTarget, fBeamDuration);
			ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectBeam(VFX_BEAM_ODD, oUser, BODY_NODE_CHEST,
					FALSE), oTarget, fBeamDuration); break;
		...
		case 64: ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_ICESTORM), lEffect);
			break;
		case 65: ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_SUNBEAM), lEffect);
			break;
			// Weatherupdate on Glaobal Weather System by Torsten Sens
			/*case 66: weather_update(WEATHER_CLEAR); break;
			 * case 67: weather_update(WEATHER_RAIN); break;
			 * case 68: weather_update(WEATHER_SNOW); break;
			 * case 69: weather_update(); break;*/
			//Summon Effects
		case 71: ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_SUMMON_MONSTER_1),
				lEffect); break;
		case 72: ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_SUMMON_MONSTER_2),
				lEffect); break;
		case 73: ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_SUMMON_MONSTER_3),
				lEffect); break;
		case 74: ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_SUMMON_CELESTIAL),
				lEffect); break;
		case 75: ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_SUMMON_GATE),
				lEffect); break;
		case 76: ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_SUMMON_UNDEAD),
				lEffect); break;
		case 77: ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_NATURES_BALANCE),
				lEffect); break;
		case 78: ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_PWKILL), lEffect);
			break;
		case 79: ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_WORD), lEffect);
			break;
			//Delayed Effects
		case 81: DelayCommand(fDelay, ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(
						VFX_FNF_SUMMON_CELESTIAL), lEffect, fDuration)); break;
		case 82: DelayCommand(fDelay, ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(
						VFX_FNF_SUMMON_GATE), lEffect, fDuration)); break;
		case 83: DelayCommand(fDelay, ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(
						VFX_FNF_NATURES_BALANCE), lEffect, fDuration)); break;
		case 84: DelayCommand(fDelay, ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(
						VFX_FNF_METEOR_SWARM), lEffect, fDuration)); break;
		case 85: DelayCommand(fDelay, ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(
						VFX_FNF_GAS_EXPLOSION_MIND), lEffect, fDuration)); break;
		case 86: DelayCommand(fDelay, ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(
						VFX_FNF_DISPEL_DISJUNCTION), lEffect, fDuration)); break;
		case 87: DelayCommand(fDelay, ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(
						VFX_FNF_HORRID_WILTING), lEffect, fDuration)); break;
		case 88: DelayCommand(fDelay, ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(
						VFX_FNF_PWKILL), lEffect, fDuration)); break;
		case 89: DelayCommand(fDelay, ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(
						VFX_FNF_WEIRD), lEffect, fDuration)); break;
			//Settings
		case 91:
			SetLocalString(oUser, "EffectSetting", "EffectDuration");
			CreateSetting(oUser);
...
// find out which object is closest to the PC
	if ( fDistanceChair < fDistanceBench
		&& fDistanceChair < fDistanceCouch && fDistanceChair < fDistanceStool ) {
		oSit = oChair; fDistSit = fDistanceChair;
	} else
	if ( fDistanceBench < fDistanceChair
		&& fDistanceBench < fDistanceCouch && fDistanceBench < fDistanceStool ) {
		oSit = oBenchPew; fDistSit = fDistanceBench;
	} else
	if ( fDistanceCouch < fDistanceChair
		&& fDistanceCouch < fDistanceBench && fDistanceCouch < fDistanceStool ) {
...
//if (fDistanceStool<fDistanceChair && fDistanceStool<fDistanceBench && fDistanceStool<fDistanceCouch)
	...
	if ( oSit !=  OBJECT_INVALID && fDistSit < 12.0 ) {
		...
		if ( GetSittingCreature(oSit) == OBJECT_INVALID ) {
			...
			SendMessageToPC(oPC, "Der naheste Stuhl wird genommen ");
		}
	} else {
		SendMessageToPC(oPC, "Es gibt keine Stühle in der Nähe");
	}
}
...
void SmokePipe(object oActivator) {
	string sEmote1 = "*pafft einige Rauchwolken in die Luft*";
	string sEmote2 = "*zieht an einer Pfeife*";
	string sEmote3 = "*nimmt einen guten Zug aus der Pfeife*";
	float fHeight = 1.7;
	float fDistance = 0.1;
	...
	// glow red
	AssignCommand(oActivator, ActionDoCommand(ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectVisualEffect(
					VFX_DUR_LIGHT_RED_5), oActivator, 0.15)));
	// wait a moment
	AssignCommand(oActivator, ActionWait(3.0));
	// puff of smoke above and in front of head
	AssignCommand(oActivator, ActionDoCommand(ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(
					VFX_FNF_SMOKE_PUFF), lAboveHead)));
...
void DoEmoteFunction(int iEmote, object oUser) {
	...
	//Neue emote-Struktur (by Poly)
	switch ( iEmote ) {
		// auf Boden sitzen
		case 1: AssignCommand(oTarget, PlayAnimation(ANIMATION_LOOPING_SIT_CROSS, 1.0, fDur)); break;
			// Hinknien
		case 2: AssignCommand(oTarget, PlayAnimation(ANIMATION_LOOPING_CUSTOM3, 1.0, fDur)); break;
			// Nach Vorne umkippen
		...
		case 5: AssignCommand(oTarget, PlayAnimation(ANIMATION_LOOPING_CUSTOM7, 1.0, fDur)); break;
			// Auf der linken Seite schlafen
		case 6: AssignCommand(oTarget, PlayAnimation(ANIMATION_LOOPING_CUSTOM8, 1.0, fDur)); break;
			// Auf den naechsten Stuhl setzen
		case 7: SitInNearestChair(oTarget); break;
		...
		case 11:
			...
			// 9 - Taetigkeiten
			// Tanzen
		case 90: AssignCommand(oTarget, PlayAnimation(ANIMATION_LOOPING_CUSTOM2, 1.0, fDur)); break;
			// Anbeten
		case 91: AssignCommand(oTarget, PlayAnimation(ANIMATION_LOOPING_WORSHIP, 1.0, fDur)); break;
			// Beten
		case 92: AssignCommand(oTarget, PlayAnimation(ANIMATION_LOOPING_MEDITATE, 1.0, fDur)); break;
			// Meditieren
		case 93: AssignCommand(oTarget, PlayAnimation(ANIMATION_LOOPING_CUSTOM1, 1.0, fDur)); break;
			// Betteln
		case 94: AssignCommand(oTarget, PlayAnimation(ANIMATION_LOOPING_TALK_PLEADING, 1.0, fDur)); break;
			// Buecken
		case 95: AssignCommand(oTarget, PlayAnimation(ANIMATION_LOOPING_GET_LOW, 1.0, fDur)); break;
			// Beugen
		case 96: AssignCommand(oTarget, PlayAnimation(ANIMATION_LOOPING_GET_MID, 1.0, fDur)); break;
			// Lesen
		case 97: AssignCommand(oTarget, ActionPlayAnimation(ANIMATION_FIREFORGET_READ, 1.0)); DelayCommand(
				3.0f, AssignCommand(oTarget, PlayAnimation(ANIMATION_FIREFORGET_READ, 1.0))); break;
			// Trinken
		case 98: AssignCommand(oTarget, ActionPlayAnimation(ANIMATION_FIREFORGET_DRINK, 1.0)); break;
			// Rauchen
		case 99: SmokePipe(oTarget); break;

			// 10 - Gefuehle
			// Betrunken
		case 101: AssignCommand(oTarget, PlayAnimation(ANIMATION_LOOPING_PAUSE_DRUNK, 1.0, fDur)); break;
			// Wuetend
		case 102: AssignCommand(oTarget, PlayAnimation(ANIMATION_LOOPING_TALK_FORCEFUL, 1.0, fDur)); break;
			// Frieren
		case 103: AssignCommand(oTarget, PlayAnimation(ANIMATION_LOOPING_CUSTOM6, 1.0)); break;
			// Zucken
		case 104: AssignCommand(oTarget, PlayAnimation(ANIMATION_LOOPING_SPASM, 1.0, fDur)); break;
			// Verrueckt
		case 105: AssignCommand(oTarget, PlayAnimation(ANIMATION_LOOPING_CUSTOM5, 1.0, fDur)); break;

		default: return;
	}

	SetLocalInt(oUser, "dmfi_univ_offset", 1);

}

/*******************************************************************************
*  Function fuer Kartenspiel (by Poly)
*******************************************************************************/
void DoCardGameFunction(int iOption, object oUser) {
	if ( GetLocalInt(oUser, "cg_iPlayerCount") > 0 && iOption > 4 ) {
		SendMessageToPC(oUser,
			"Ihr duerft Eure Entscheidung nicht aendern, nachdem Ihr Spieler eingeladen habt");
	} else {
		switch ( iOption ) {
			//Spiel starten
			case 3:
				...
				//Reset
			case 4:
				resetCards(OBJECT_SELF);
				break;
				//Poker waehlen
			case 11:
				SetLocalInt(oUser, "cg_iGame", 0);
				break;

				//Blackjack waehlen
			case 12: SetLocalInt(oUser, "cg_iGame", 1);
				break;
				//Einsatz
			case 21: DeleteLocalInt(oUser, "cg_iBet"); break;
			case 22: SetLocalInt(oUser, "cg_iBet", 1); break;
			case 23: SetLocalInt(oUser, "cg_iBet", 5); break;
			case 24: SetLocalInt(oUser, "cg_iBet", 10); break;
			case 25: SetLocalInt(oUser, "cg_iBet", 50); break;
			case 26: SetLocalInt(oUser, "cg_iBet", 100); break;
			default: return;
		}

		//switch
	} //else
	SetLocalInt(oUser, "dmfi_univ_offset", 14);
}

/*******************************************************************************
*  Function fuer Kartenspiel (by Poly)
*******************************************************************************/
void DoCardPlayFunction1(int iOption, object oUser) {
	object oCardStack = GetLocalObject(oUser, "cg_oCardStack");

	if ( iOption <= 5 ) {
		SwapCard(oCardStack, iOption);
	} else {
		switch ( iOption ) {
			//Naechster Spieler
			case 6:
				ChangeCards(oCardStack);
				break;
				//Abbrechen
			case 71:
				...
				break;
				//nicht Abbrechen
			case 72: break; //do Nothing
			default: return;
		}
	}
	SetLocalInt(oUser, "dmfi_univ_offset", 15);
}

/*******************************************************************************
*  Function fuer Kartenspiel (by Poly)
*******************************************************************************/
void DoCardPlayFunction2(int iOption, object oUser) {
	object oCardStack = GetLocalObject(oUser, "cg_oCardStack");
	int iActivPlayer = GetLocalInt(oCardStack, "cg_iActivePlayer");
	switch ( iOption ) {
		case 1:
			ChangeCards(oCardStack);
			break;
			...
			//Passen
		case 2:
			SetLocalInt(oCardStack, "cg_iHold_" + IntToString(iActivPlayer), TRUE);
			DelayCommand(1.0, NextPlayer(oCardStack));
			break;
...
*  Function fuer Kartenspiel (by Poly)
*******************************************************************************/
void DoCardRequestFunction(int iOption, object oUser) {
	object oRequester = GetLocalObject(oUser, "cg_oRequester");
	if ( oRequester == OBJECT_INVALID ) return;

	int iPlayerCount = GetLocalInt(oRequester, "cg_iPlayerCount");
	if ( GetLocalInt(oRequester, "cg_iCG") == 1 ) {
		SendMessageToPC(oUser, "Ihr koennt nicht mehr beitreten, das Spiel hat bereits begonnen.");
	} else {
		switch ( iOption ) {
			//Annehmen
			case 1:
				//fuegt den gefragten Spieler zur Liste hinzu
				if ( iPlayerCount < 4 ) {
					iPlayerCount++;
					SetLocalObject(oRequester, "cg_oPlayer" + IntToString(iPlayerCount), GetPCSpeaker());
					SetLocalInt(oRequester, "cg_iPlayerCount", iPlayerCount);
					//Spieler ist im Spiel
					SetLocalInt(oUser, "cg_iCG", TRUE);
					SetLocalInt(oRequester, "cg_iCG", 2); //Kartengeber
					SendMessageToPC(oRequester, GetName(oUser) + " hat die Herauforderung angenommen.");
				} else {
					SendMessageToPC(oUser, "Tut mir leid, aber die Spielrunde ist bereits komplett.");
					SendMessageToPC(oRequester, GetName(oUser) +
						" konnte nicht beitreten, weil bereits 5 Spieler mitspielen.");
				}
				break;
				//Ablehnen
			case 2:
				SendMessageToPC(oRequester, GetName(oUser) + " hat die Herauforderung abgelehnt.");
				break;
			default: return;
		}
	}
	DeleteLocalObject(oUser, "cg_oRequester");
}
...
void main() {
	...
	else if ( sDMFI == "pc_cards" )
		DoCardGameFunction(iDMFI, OBJECT_SELF);
	else if ( sDMFI == "pc_cards1" )
		DoCardPlayFunction1(iDMFI, OBJECT_SELF);
	else if ( sDMFI == "pc_cards2" )
		DoCardPlayFunction2(iDMFI, OBJECT_SELF);
	else if ( sDMFI == "pc_cards_req" )
		DoCardRequestFunction(iDMFI, OBJECT_SELF);
	//
	else if ( sDMFI == "fx" )
