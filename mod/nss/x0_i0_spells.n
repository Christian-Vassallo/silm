...
void spellsDispelMagic(object oTarget, int nCasterLevel, effect eVis, effect eImpac, int bAll = TRUE,
...
void spellsInflictTouchAttack(int nDamage, int nMaxExtraDamage, int nMaximized, int vfx_impactHurt, int vfx_impactHeal,
							  int nSpellID);
...
void DoGrenade(int nDirectDamage, int nSplashDamage, int vSmallHit, int vRingHit, int nDamageType, float fExplosionRadius, int nObjectFilter,
...
//    int nMetaMagic = GetMetaMagicFeat();
	...
	if ( spellsIsTarget(oTarget, SPELL_TARGET_SELECTIVEHOSTILE, GetAreaOfEffectCreator()) ) {
		...
		if ( !MyResistSpell(GetAreaOfEffectCreator(), oTarget) ) {
			...
			if ( !MySavingThrow(SAVING_THROW_FORT, oTarget, GetSpellSaveDC(), SAVING_THROW_ALL,
	...
	if ( spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, GetAreaOfEffectCreator()) ) {
		...
		if ( !MyResistSpell(GetAreaOfEffectCreator(), oTarget, fDelay) ) {
			...
			if ( GetHasSpellEffect(SPELL_SPIKE_GROWTH, oTarget) == FALSE ) {
				...
				if ( !MySavingThrow(SAVING_THROW_REFLEX, oTarget, GetSpellSaveDC(), SAVING_THROW_ALL,
	...
	if ( GetRacialType(oTarget) == RACIAL_TYPE_UNDEAD ) {
		...
		if ( !GetIsReactionTypeFriendly(oTarget) ) {
			...
			if ( !MyResistSpell(OBJECT_SELF, oTarget) ) {
				...
				if ( MySavingThrow(SAVING_THROW_WILL, oTarget, GetSpellSaveDC(), SAVING_THROW_ALL,
						OBJECT_SELF) ) {
...
//    int nDamage = 0;
	...
	while ( GetIsObjectValid(oTarget) && nCnt <= nEnemies ) {
		// * caster cannot be harmed by this spell
		if ( spellsIsTarget(oTarget, SPELL_TARGET_SELECTIVEHOSTILE, OBJECT_SELF)
			&& ( oTarget != OBJECT_SELF ) && ( GetObjectSeen(oTarget, OBJECT_SELF) ) ) {
			//Fire cast spell at event for the specified target
			...
			if ( !MyResistSpell(OBJECT_SELF, oTarget, fDelay) ) {
				for ( i = 1; i <= nExtraMissiles + nRemainder; i++ ) {
					...
					if ( nReflexSave ) {
						nDam = GetReflexAdjustedDamage(nDam, oTarget, GetSpellSaveDC(),
		...
		if ( spellsIsTarget(oTarget, nTargetType, oCaster) == TRUE ) {
			...
			if ( !nResistSpellSuccess ) {
				...
				if ( nSavingThrowType != SAVING_THROW_NONE ) {
					nSavingThrowSuccess = MySavingThrow(nSavingThrowType, oTarget, GetSpellSaveDC(),
											  nSavingThrowSubType);
				}
...
void spellApplyMindBlank(object oTarget, int nSpellId, float fDelay = 0.0) {
	...
	DelayCommand(fDelay, ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eLink, oTarget, TurnsToSeconds(
				nDuration)));
...
void RemoveSpellEffects2(int nSpell_ID, object oCaster, object oTarget, int nSpell_ID2, int nSpell_ID3) {
	...
	if ( GetHasSpellEffect(nSpell_ID, oTarget)
		|| GetHasSpellEffect(nSpell_ID2, oTarget) || GetHasSpellEffect(nSpell_ID3, oTarget) ) {
