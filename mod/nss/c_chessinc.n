...
void CheckBishop();
void MovePiece(int nOldPosition, int nNewPosition, int nTake = FALSE);
...
string MovePositions(int nStartPos, int nEndPos, string sMiddle = "-");
...
// Makes Gamemaster and both kings say something.
void Announce(string str);

// Appends something to the gamelog
void GameLog(object oFigure, int iPosStart, int iPosEnd, int nTake = FALSE, int nCheck = FALSE, int nCheckMate = FALSE, string sAppend = "",
			 object oChangeFig = OBJECT_INVALID);
void GameLog2(string act);
void GetFigureString(object o);
...
void MovePiece(int nOldPosition, int nNewPosition, int nTake = FALSE) {
	...
	if ( CheckForCheck(nKingPos, nTurn) ) {
		...
		ActionSpeakString("Nein, der König wäre in Schach, würde ich dorthin gehen!");
		return;
	}
	...
	if ( nCheckMate == 2 ) {
		...
		if ( nLogCheck = CheckForCheck(nKingPos, nTurn) )
			SpeakString("Schach!", TALKVOLUME_TALK);
	...
	// move: (figure || "" if pawn)(start)[-x](end)[x# || figure for change]

	GameLog(OBJECT_SELF, nOldPosition, nNewPosition, nTake, nLogCheck, nEndGame);
...
void TakePiece(int nOldPosition, int nNewPosition) {
	...
	nTurn = GetLocalInt(oGM, "Turn");
	MovePiece(nOldPosition, nNewPosition, TRUE);
...
int CheckForCheckMate() {
	...
	while ( ( x < 64 ) && ( nCheck == 0 ) ) {
		...
		if ( nTurn * nTemp > 0 ) {
			...
			switch ( abs(nTemp) ) {
				case 1: {
						ExecuteScript("c_CHECKPAWN", oTemp);
				...
				case 2: {
						ExecuteScript("c_CHECKROOK", oTemp);
				...
				case 3: {
						ExecuteScript("c_CHECKKNIGHT", oTemp);
				...
				case 4: {
						ExecuteScript("c_CHECKBISHOP", oTemp);
			...
			for ( z = 0; z < 64; z++ ) {
				...
				switch ( nTemp * nTurn ) {
					...
					case 4: {
							...
							if ( abs(nxChecker - x) == abs(nyChecker - y) ) {
								if ( nxChecker > x ) dx = 1;
								else dx = -1;
								if ( nyChecker > y ) dy = 1;
								else dy = -1;
...
string MovePositions(int nStartPos, int nEndPos, string sMiddle = "-") {
	...
	if ( ( nStartPos > 63 ) || ( nStartPos < 0 ) || ( nEndPos > 63 ) || ( nEndPos < 0 ) )
		...
		sTemp = sTemp + sMiddle + GetStringArray(oGM, "sNotation", nEndPos);
	}
	return sTemp;
}
...
void StartFight(object oTarget) {
	...
	SetLocalInt(GetNearestObjectByTag("c_gamemaster"), "piecefight", 1);
...
void PromotePawn(string sNewPiece, int nNewPiece) {
	...
	if ( GetLocalInt(OBJECT_SELF, "nPiece") > 0 ) {
		oTemp = CreateObject(OBJECT_TYPE_CREATURE, "c_" + sNewPiece + "_w", lLoc);
		SetIntArray(oGM, "nSquare", nPosition, nNewPiece);
	} else {
		oTemp = CreateObject(OBJECT_TYPE_CREATURE, "c_" + sNewPiece + "_b", lLoc);
		SetIntArray(oGM, "nSquare", nPosition, ( -nNewPiece ));
	}
	...
	ExecuteScript("c_heartbeat", oTemp);
	...
	if ( nCheckMate == 2 ) {
		...
		if ( nCheck = CheckForCheck(nKingPos, nTurn) ) AssignCommand(oTemp, SpeakString("Schach!",
					TALKVOLUME_TALK));
	}

	GameLog(OBJECT_SELF, GetLocalInt(OBJECT_SELF, "nPosition"), nPosition,
		GetIsObjectValid(GetLocalObject(oTemp, "target")), nCheck, nEndGame, "", oTemp);
...
void Announce(string str) {
	object
	oGM = GetNearestObjectByTag("c_gamemaster"),
	oWhiteKing = GetNearestObjectByTag("c_king_w"),
	oBlackKing = GetNearestObjectByTag("c_king_b"),
	oWP = GetLocalObject(oGM, "oWhitePlayer"),
	oBP = GetLocalObject(oGM, "oBlackPlayer");


	if ( GetIsObjectValid(oGM) )
		AssignCommand(oGM, ActionSpeakString(str));

	if ( GetIsObjectValid(oWhiteKing) )
		AssignCommand(oWhiteKing, ActionSpeakString(str));
	if ( GetIsObjectValid(oBlackKing) )
		AssignCommand(oBlackKing, ActionSpeakString(str + " *grunz*"));

	if ( GetIsObjectValid(oWP) )
		SendMessageToPC(oWP, "Der Spielmeister sagt: " + str);

	if ( GetIsObjectValid(oBP) )
		SendMessageToPC(oBP, "Der Spielmeister sagt: " + str);
}
...
void GameLog(object oFigure, int iPosStart, int iPosEnd, int nTake = FALSE, int nCheck = FALSE, int nCheckMate = FALSE, string sAppend = "",
	...
	string res = "";
	string sFigureName = "";
	string sChangeFigureName = "";

	if ( "pawn" == GetStringLeft(GetResRef(oFigure), 4) )
		sFigureName = "";
	if ( "rook" == GetStringLeft(GetResRef(oFigure), 4) )
		sFigureName = "T";
	if ( "bishop" == GetStringLeft(GetResRef(oFigure), 6) )
		sFigureName = "L";
	if ( "king" == GetStringLeft(GetResRef(oFigure), 4) )
		sFigureName = "K";
	if ( "queen" == GetStringLeft(GetResRef(oFigure), 5) )
		sFigureName = "D";
	if ( "knight" == GetStringLeft(GetResRef(oFigure), 6) )
		sFigureName = "S";

	if ( GetIsObjectValid(oChangeFig) ) {
		if ( "pawn" == GetStringLeft(GetResRef(oChangeFig), 4) )
			sChangeFigureName = "";
		if ( "rook" == GetStringLeft(GetResRef(oChangeFig), 4) )
			sChangeFigureName = "T";
		if ( "bishop" == GetStringLeft(GetResRef(oChangeFig), 6) )
			sChangeFigureName = "L";
		if ( "king" == GetStringLeft(GetResRef(oChangeFig), 4) )
			sChangeFigureName = "K";
		if ( "queen" == GetStringLeft(GetResRef(oChangeFig), 5) )
			sChangeFigureName = "D";
		if ( "knight" == GetStringLeft(GetResRef(oChangeFig), 6) )
			sChangeFigureName = "S";
	}
	...
	res +=
		sFigureName +
		MovePositions(iPosStart, iPosEnd, nTake ? "x" : "-") +
		( nCheck ? "+" : ( nCheckMate ? "#" : "" ) ) +
		sChangeFigureName +
		sAppend;
...
void GameLog2(string act) {
	object oGM = GetNearestObjectByTag("c_gamemaster");
	if ( GetIsObjectValid(oGM) )
		SetLocalString(oGM, "GameLog", GetLocalString(oGM, "GameLog") + " " + act);
	else
