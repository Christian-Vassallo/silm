...
void main() {
	...
	//Hilfe zum erfassen von Placeables mittels SL-Stab (by Poly)
	if ( sItemTag == "dmfi_dmw" && !GetIsObjectValid(oOther) ) {
		oOther = GetNearestObjectToLocation(OBJECT_TYPE_PLACEABLE, lLocation);
		//Ausgabe des Targets an den SL/GM (by Poly)
		SendMessageToPC(oUser, "gewaehltes Objekt: " + GetName(oOther));
	}
	//
	...
	if ( GetStringLeft(sItemTag, 8) == "hlslang_" ) {
		...
		DelayCommand(1.0f, FloatingTextStringOnCreature("You are speaking " +
				GetName(oItem) + ". Type /dm [(what you want to say in brackets)]", oUser, FALSE));
	...
	if ( GetStringLeft(sItemTag, 8) == "dmfi_pc_" ) {
		...
		if ( sItemTag == "dmfi_pc_cards" ) {
			if ( GetLocalInt(oUser, "cg_iCG") == 1 ) {
				SendMessageToPC(oUser, "Du bist bereits in einer Spielrunde.");
				return;
			}
			if ( GetIsPC(oOther) && oOther != oUser ) {
				// Wenn ein anderer Spieler gewaehlt wird...
				if ( GetLocalInt(oOther, "cg_iCG") != 0 ) {
					SendMessageToPC(oUser, GetName(oOther) + " ist bereits in einer Spielrunde.");
					return;
				}
				if ( GetLocalInt(oUser, "cg_iPlayerCount") == 4 ) {
					SendMessageToPC(oUser, "Die Spielrunde ist bereits voll.");
					return;
				}
				SetLocalObject(oOther, "cg_oRequester", oUser);
				SetLocalObject(oUser, "dmfi_univ_target", oOther);
				SetLocalString(oOther, "dmfi_univ_conv", GetStringRight(sItemTag, GetStringLength(sItemTag) -
						5) + "_req");
				AssignCommand(oOther, ClearAllActions());
				AssignCommand(oOther, ActionStartConversation(oOther, "dmfi_universal", TRUE));
				return;
			}
		}
		//
	...
	if ( GetStringLeft(sItemTag, 5) == "dmfi_" ) {
		...
		if ( sItemTag == "dmfi_exploder" ) {
			if ( !GetIsObjectValid(GetItemPossessedBy(oUser, "dmfi_afflict")) ) CreateItemOnObject(
					"dmfi_afflict", oUser);
			if ( !GetIsObjectValid(GetItemPossessedBy(oUser, "dmfi_dicebag")) ) CreateItemOnObject(
					"dmfi_dicebag", oUser);
			if ( !GetIsObjectValid(GetItemPossessedBy(oUser, "dmfi_pc_dicebag")) ) CreateItemOnObject(
					"dmfi_pc_dicebag", oUser);
			if ( !GetIsObjectValid(GetItemPossessedBy(oUser, "dmfi_pc_follow")) ) CreateItemOnObject(
					"dmfi_pc_follow", oUser);
			if ( !GetIsObjectValid(GetItemPossessedBy(oUser, "dmfi_pc_emote")) ) CreateItemOnObject(
					"dmfi_pc_emote", oUser);
			if ( !GetIsObjectValid(GetItemPossessedBy(oUser, "dmfi_dmw")) ) CreateItemOnObject("dmfi_dmw",
					oUser);
			if ( !GetIsObjectValid(GetItemPossessedBy(oUser, "dmfi_emote")) ) CreateItemOnObject("dmfi_emote",
					oUser);
			if ( !GetIsObjectValid(GetItemPossessedBy(oUser, "dmfi_encounter")) ) CreateItemOnObject(
					"dmfi_encounter", oUser);
			if ( !GetIsObjectValid(GetItemPossessedBy(oUser, "dmfi_faction")) ) CreateItemOnObject(
					"dmfi_faction", oUser);
			if ( !GetIsObjectValid(GetItemPossessedBy(oUser, "dmfi_fx")) ) CreateItemOnObject("dmfi_fx",
					oUser);
			if ( !GetIsObjectValid(GetItemPossessedBy(oUser, "dmfi_music")) ) CreateItemOnObject("dmfi_music",
					oUser);
			if ( !GetIsObjectValid(GetItemPossessedBy(oUser, "dmfi_sound")) ) CreateItemOnObject("dmfi_sound",
					oUser);
			if ( !GetIsObjectValid(GetItemPossessedBy(oUser, "dmfi_voice")) ) CreateItemOnObject("dmfi_voice",
					oUser);
			if ( !GetIsObjectValid(GetItemPossessedBy(oUser, "dmfi_xp")) ) CreateItemOnObject("dmfi_xp",
					oUser);
			if ( !GetIsObjectValid(GetItemPossessedBy(oUser, "dmfi_500xp")) ) CreateItemOnObject("dmfi_500xp",
					oUser);
			if ( !GetIsObjectValid(GetItemPossessedBy(oUser, "dmfi_en_ditto")) ) CreateItemOnObject(
					"dmfi_en_ditto", oUser);
			if ( !GetIsObjectValid(GetItemPossessedBy(oUser, "dmfi_mute")) ) CreateItemOnObject("dmfi_mute",
					oUser);
			if ( !GetIsObjectValid(GetItemPossessedBy(oUser, "dmfi_peace")) ) CreateItemOnObject("dmfi_peace",
					oUser);
			if ( !GetIsObjectValid(GetItemPossessedBy(oUser, "dmfi_voicewidget")) ) CreateItemOnObject(
					"dmfi_voicewidget", oUser);
			return;
		}
		...
		if ( sItemTag == "dmfi_voicewidget" ) {
			...
			if ( GetIsObjectValid(oOther) ) {
				...
				FloatingTextStringOnCreature("You have targeted " +
					GetName(oOther) + " with the Voice Widget", oUser, FALSE);
				object oArea = GetFirstObjectInArea(GetArea(oUser));
				while ( GetIsObjectValid(oArea) ) {
					if ( GetObjectType(oArea) == OBJECT_TYPE_CREATURE
						&& GetLocalInt(oArea, "hls_Listening")
