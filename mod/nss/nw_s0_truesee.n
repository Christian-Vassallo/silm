...
// End of Spell Cast Hook
	...
	float fDurMul = GetLocalFloat(oTarget, "d_m");
	if ( fDurMul <= 0.0 )
		fDurMul = 1.0;
	nDuration = FloatToInt(IntToFloat(nDuration) * fDurMul);

	//Apply the VFX impact and effects
	ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eLink, oTarget, TurnsToSeconds(nDuration));

	SetupReminder(OBJECT_SELF, "Wahrer Blick", TurnsToSeconds(nDuration), eLink);
}
