...
int InvisibleBecome(object oSelf = OBJECT_SELF) {
	...
	if ( GetHasSpell(SPELL_IMPROVED_INVISIBILITY) || GetHasSpell(SPELL_INVISIBILITY)
		...
		if ( InvisibleTrue(oSelf) == FALSE ) {
			...
			object oSeeMe = GetNearestCreature(CREATURE_TYPE_REPUTATION, REPUTATION_TYPE_ENEMY, OBJECT_SELF,
								1, CREATURE_TYPE_PERCEPTION, PERCEPTION_SEEN, CREATURE_TYPE_HAS_SPELL_EFFECT,
								SPELL_TRUE_SEEING);
			if ( !GetIsObjectValid(oSeeMe) ) {
				oSeeMe = GetNearestCreature(CREATURE_TYPE_REPUTATION, REPUTATION_TYPE_ENEMY, OBJECT_SELF, 1,
...
int InvisibleTrue(object oSelf = OBJECT_SELF) {
	if ( GetHasEffect(EFFECT_TYPE_INVISIBILITY, oSelf)
		|| GetHasEffect(EFFECT_TYPE_IMPROVEDINVISIBILITY, oSelf)
...
int GetShouldNotCastSpellsBecauseofArmor(object oTarget, int nClass) {
	...
	if ( GetArcaneSpellFailure(oTarget) > 5
		&& ( nClass == CLASS_TYPE_SORCERER || nClass == CLASS_TYPE_WIZARD ) ) {
		return TRUE;
	}
	return FALSE;
}
...
int chooseTactics(object oIntruder) {
	...
	if ( GetHasFeatEffect(FEAT_BARBARIAN_RAGE) == TRUE
		|| GetShouldNotCastSpellsBecauseofArmor(OBJECT_SELF, nClass) == TRUE
...
int __InCombatRound() {
	...
	if ( nCurrentAction == ACTION_ATTACKOBJECT
		|| nCurrentAction == ACTION_INVALID || nCurrentAction == ACTION_MOVETOPOINT ) {
		return FALSE;
	}
...
object GetLockedObject(object oMaster) {
	int nCnt = 1;
	int bValid = TRUE;
	object oLastObject = GetNearestObjectToLocation(OBJECT_TYPE_DOOR |
							 OBJECT_TYPE_PLACEABLE, GetLocation(oMaster), nCnt);
	while ( GetIsObjectValid(oLastObject) && bValid == TRUE ) {
		...
		oLastObject = GetNearestObjectToLocation(OBJECT_TYPE_DOOR |
						  OBJECT_TYPE_PLACEABLE, GetLocation(oMaster), nCnt);
	}
...
// Get the nearest seen neutral creature.
object GetNearestSeenNeutral(object oSource = OBJECT_SELF, int nNth = 1) {
...
void DetermineSpecialBehavior(object oIntruder = OBJECT_INVALID) {
	//first focus oneself on enemy creatures, then maybe on any passersby
	object oTarget = GetNearestSeenEnemy();

	//Only omnivores (possible food) and shy creatures (possible threat)
	//put neutral creatures into regard.
	if ( !GetBehaviorState(NW_FLAG_BEHAVIOR_HERBIVORE)
		&& !GetIsObjectValid(oTarget) )
		oTarget = GetNearestSeenNeutral();
	...
	if ( GetBehaviorState(NW_FLAG_BEHAVIOR_OMNIVORE) ) {
		...
		//If we're under direct attack, defend oneself, otherwise,
		//try to flee from the intruder if it's no druid or ranger
		//stand still on nature guys,
		//if completely undisturbed, wander around.
		if ( !GetIsObjectValid(GetAttemptedAttackTarget())
			&& !GetIsObjectValid(GetAttemptedSpellTarget())
			&& !GetIsObjectValid(GetAttackTarget()) ) {
...
int BashDoorCheck(object oIntruder = OBJECT_INVALID) {
	...
	if ( GetIsObjectValid(oDoor) ) {
		...
		if ( !GetIsObjectValid(GetNearestCreature(CREATURE_TYPE_REPUTATION, REPUTATION_TYPE_ENEMY,
