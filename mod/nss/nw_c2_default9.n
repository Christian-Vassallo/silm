...
void main() {
	...
	string sTag = GetStringLowerCase(GetTag(OBJECT_SELF));
	...
	if ( GetLocalInt(OBJECT_SELF, "wander") )
		SetSpawnInCondition(NW_FLAG_AMBIENT_ANIMATIONS);

	// XXX FIX TODO Breaks WalkWayPoints?
	else if ( CheckWayPoints() )
		SetSpawnInCondition(NW_FLAG_IMMOBILE_AMBIENT_ANIMATIONS);
	...
	if ( GetLocalInt(OBJECT_SELF, "wanderclose") )
		SetAnimationCondition(NW_ANIM_FLAG_IS_MOBILE_CLOSE_RANGE);
	...
	// Geisterhafter Effect auf Geisterwesen setzen
	string sEffect = GetStringLowerCase(GetLocalString(OBJECT_SELF, "Effect"));
	effect eVis;

	if ( sTag == "geisterhaft" || sEffect == "geisterhaft" ) {
		eVis = SupernaturalEffect(EffectVisualEffect(VFX_DUR_GHOSTLY_VISAGE));
		ApplyEffectToObject(DURATION_TYPE_PERMANENT, eVis, OBJECT_SELF);
	}

	if ( sEffect == "geisterhaftw" ) {
		ApplyEffectToObject(DURATION_TYPE_PERMANENT, SupernaturalEffect(EffectVisualEffect(
					VFX_DUR_GHOST_TRANSPARENT)), OBJECT_SELF);
		ApplyEffectToObject(DURATION_TYPE_PERMANENT, SupernaturalEffect(EffectVisualEffect(
					VFX_DUR_GHOST_SMOKE)), OBJECT_SELF);
		ApplyEffectToObject(DURATION_TYPE_PERMANENT, SupernaturalEffect(EffectVisualEffect(VFX_DUR_AURA_WHITE)),
			OBJECT_SELF);
		ApplyEffectToObject(DURATION_TYPE_PERMANENT, SupernaturalEffect(EffectVisualEffect(VFX_DUR_GLOW_WHITE)),
			OBJECT_SELF);
		ApplyEffectToObject(DURATION_TYPE_PERMANENT, SupernaturalEffect(EffectVisualEffect(
					VFX_DUR_LIGHT_GREY_5)), OBJECT_SELF);
	}

	// Reitgeschwindigkeit und Hufgerusche auf Reitenden NSC setzen
	if ( GetIsRidingHorse(OBJECT_SELF) )
		ApplyRidingEffects(OBJECT_SELF);

	// sTask
	string sTask = GetLocalString(OBJECT_SELF, "Task");
	if ( sTask == "meditieren" ) {
		AssignCommand(OBJECT_SELF, ActionPlayAnimation(ANIMATION_LOOPING_MEDITATE, 1.0, 500000.0));
	} else if ( sTask == "sitzen" ) {
		AssignCommand(OBJECT_SELF, ActionPlayAnimation(ANIMATION_LOOPING_SIT_CROSS, 1.0, 500000.0));
	} else if ( sTask == "diskutieren" ) {
		AssignCommand(OBJECT_SELF, ActionPlayAnimation(ANIMATION_LOOPING_TALK_FORCEFUL, 1.0, 500000.0));
	} else if ( sTask == "buecken" ) {
		AssignCommand(OBJECT_SELF, ActionPlayAnimation(ANIMATION_LOOPING_GET_LOW, 1.0, 500000.0));
	} else if ( sTask == "arbeiten" ) {
		AssignCommand(OBJECT_SELF, ActionPlayAnimation(ANIMATION_LOOPING_GET_MID, 1.0, 500000.0));
	} else if ( sTask == "liegen" ) {
		AssignCommand(OBJECT_SELF, ActionPlayAnimation(ANIMATION_LOOPING_DEAD_FRONT, 1.0, 500000.0));
	} else if ( sTask == "beten" ) {
		AssignCommand(OBJECT_SELF, ActionPlayAnimation(ANIMATION_LOOPING_WORSHIP, 1.0, 500000.0));
	}

	if ( GetTag(OBJECT_SELF) == "ascore_ghost" ) {
		SetName(OBJECT_SELF, "Ascore-Geist");
	}

	if ( GetTag(OBJECT_SELF) == "irrlicht_gw" ) {
		ActionRandomWalk();
		SetCommandable(0);
	...
	int n = 0, vfx = 0;
	while ( ( vfx = GetLocalInt(OBJECT_SELF, "vfx" + IntToString(n)) ) > 0 ) {
		ApplyEffectToObject(DTP, SupernaturalEffect(EffectVisualEffect(vfx)), OBJECT_SELF);
		n++;
	}
}
