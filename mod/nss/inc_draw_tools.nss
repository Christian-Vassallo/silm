...
void GroupApplyEffectToObject(int nDurationType, effect eEffect, object oData, float fDuration = 0.0f, float fOvertime = 3.0f,
							  int bReverseOrder = FALSE);
...
void GroupSetFacing(float fDirection, object oData, int bRelative = TRUE, float fOvertime = 3.0f,
					int bReverseOrder = FALSE);
...
void GroupPlayAnimation(int nAnimation, object oData, float fSpeed = 1.0f, float fOvertime = 3.0f,
						int bReverseOrder = FALSE);
...
void GroupExecuteScript(string sScript, object oData, float fOvertime = 3.0f, int bReverseOrder = FALSE) {
	...
	if ( bReverseOrder ) {
		...
		for ( i = nTotal - 1; i > -1; i-- ) {
			DelayCommand(fBreak *
				IntToFloat(j), ExecuteScript(sScript, GetLocalObject(oData, "store" + IntToString(i))));
			j++;
		}
	} else {
		for ( i = 0; i < nTotal; i++ ) {
			DelayCommand(fBreak *
				IntToFloat(i), ExecuteScript(sScript, GetLocalObject(oData, "store" + IntToString(i))));
		}
	}
}
...
void GroupDestroyObject(object oData, float fDelay = 0.0f, float fOvertime = 3.0f, int bReverseOrder = FALSE)
{
	...
	if ( bReverseOrder ) {
		...
		for ( i = nTotal - 1; i > -1; i-- ) {
			DelayCommand(fDelay +
				fBreak * IntToFloat(j), DestroyObject(GetLocalObject(oData, "store" + IntToString(i))));
			j++;
		}
	} else {
		for ( i = 0; i < nTotal; i++ ) {
			DelayCommand(fDelay +
				fBreak * IntToFloat(i), DestroyObject(GetLocalObject(oData, "store" + IntToString(i))));
		}
	}
...
void GroupApplyEffectToObject(int nDurationType, effect eEffect, object oData, float fDuration = 0.0f, float fOvertime = 3.0f,
	...
	if ( bReverseOrder ) {
		...
		for ( i = nTotal - 1; i > -1; i-- ) {
			DelayCommand(fBreak *
				IntToFloat(j), ApplyEffectToObject(nDurationType, eEffect, GetLocalObject(oData, "store" +
						IntToString(i)), fDuration));
			j++;
		}
	} else {
		for ( i = 0; i < nTotal; i++ ) {
			DelayCommand(fBreak *
				IntToFloat(i), ApplyEffectToObject(nDurationType, eEffect, GetLocalObject(oData, "store" +
						IntToString(i)), fDuration));
		}
	}
}
	...
	if ( bReverseOrder ) {
		...
		if ( bRelative ) {
			...
			for ( i = nTotal - 1; i > -1; i-- ) {
				...
				DelayCommand(fBreak *
					IntToFloat(j), AssignCommand(oNode, SetFacing(GetFacing(oNode) + fDirection)));
				j++;
			}
		} else {
			for ( i = nTotal - 1; i > -1; i-- ) {
				DelayCommand(fBreak *
					IntToFloat(j), AssignCommand(GetLocalObject(oData, "store" +
							IntToString(i)), SetFacing(fDirection)));
				j++;
			}
		}
	} else {
		if ( bRelative ) {
			...
			for ( i = 0; i < nTotal; i++ ) {
				oNode = GetLocalObject(oData, "store" + IntToString(i));
				DelayCommand(fBreak *
					IntToFloat(i), AssignCommand(oNode, SetFacing(GetFacing(oNode) + fDirection)));
			}
		} else {
			for ( i = 0; i < nTotal; i++ ) {
				DelayCommand(fBreak *
					IntToFloat(i), AssignCommand(GetLocalObject(oData, "store" +
							IntToString(i)), SetFacing(fDirection)));
			}
		}
	}
...
void GroupSetFacingPoint(vector vTarget, object oData, float fOvertime = 3.0f, int bReverseOrder = FALSE) {
	...
	if ( bReverseOrder ) {
		...
		for ( i = nTotal - 1; i > -1; i-- ) {
			DelayCommand(fBreak *
				IntToFloat(j), AssignCommand(GetLocalObject(oData, "store" +
						IntToString(i)), SetFacingPoint(vTarget)));
			j++;
		}
	} else {
		for ( i = 0; i < nTotal; i++ ) {
...
void GroupPlayAnimation(int nAnimation, object oData, float fSpeed = 1.0f, float fOvertime = 3.0f,
	...
	if ( bReverseOrder ) {
		...
		for ( i = nTotal - 1; i > -1; i-- ) {
			DelayCommand(fBreak *
				IntToFloat(j), AssignCommand(GetLocalObject(oData, "store" +
						IntToString(i)), PlayAnimation(nAnimation, fSpeed)));
			j++;
		}
	} else {
		for ( i = 0; i < nTotal; i++ ) {
