...
// End of Spell Cast Hook
    ...
    int nTargets = GetCasterLevel(OBJECT_SELF) / 4;
    int nDuration = GetCasterLevel(OBJECT_SELF) * 60 * 10;
    if(nTargets == 0)
    {
        nTargets = 1;
    }
    ...
    while(GetIsObjectValid(oTarget) && nTargets != 0)
    {
        if(GetIsFriend(oTarget) && OBJECT_SELF != oTarget)
        {
            ...
            DelayCommand(fDelay, ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eLink, oTarget, IntToFloat(nDuration)));
            nTargets--;
        }
    ...
    DelayCommand(fDelay, ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eLink, OBJECT_SELF, IntToFloat(nDuration)));
	
	SetupReminder(OBJECT_SELF, GetSpellName(GetSpellId()), IntToFloat(nDuration), eLink);
