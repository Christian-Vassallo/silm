...
void ApplyPenalty(object oPC) {
	int iHowMuch = GetXP(oPC) / 20;

	if ( iHowMuch > GetFreeXP(oPC) ) iHowMuch = GetFreeXP(oPC);

	//Half of it at once...
	XP_LoseXP(oPC, iHowMuch / ( 5 * 2 ));

	//The other half in case he respawns instead of being resurrected.
	SetLegacyPersistentInt(oPC, "PC_XP_PENALTY", iHowMuch / ( 5 * 3 ));
}
...
void main() {
	...
	object oKnockedBy = GetLocalObject(oPlayer, "KILLER_WAS");
...
// * MAY 2002: Just popup the YOU ARE DEAD panel at this point
	...
	SetLegacyPersistentInt(oPlayer, "PC_XP_PENALTY", 0);

	if ( GetIsPC(oKiller) && oKiller != oPlayer
		&& GetIsPC(oKnockedBy) && oKnockedBy != oPlayer ) {
		SendMessageToPC(oPlayer, "PvP. Notiz an DMs wird gesendet.");
		SendMessageToAllDMs("PvP. " +
			GetName(oPlayer) +
			" wurde niedergeschlagen von " +
			GetName(oKnockedBy) + " und getoetet von " + GetName(oKiller) + ".");
	} else {
		ApplyPenalty(oPlayer);
	}

	RefreshSubraceEffects(oPlayer);
	ExecuteScript("corpse_pc_death", oPlayer);
