...
void spellsCreateItemForSummoned(object oCaster, float fDuration) {
	...
	if ( GetIsObjectValid(oSummon) ) {
		...
		AddItemProperty(DURATION_TYPE_TEMPORARY, ItemPropertyDamageReduction(IP_CONST_DAMAGEREDUCTION_1, 5),
			oWeapon, fDuration);
	}
}
...
void main() {
	...
	DelayCommand(1.0, spellsCreateItemForSummoned(oSelf, TurnsToSeconds(nDuration * 10)));
}
