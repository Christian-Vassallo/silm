...
void main() {
	...
	} else if ( 1 == nMenuLevel0 || 2 == nMenuLevel0 ) {
		...
		if ( 0 == nMenuLevel1 ) {
			switch ( nSelected ) {
				...
				case 2: // 25
					...
					break;
				case 3: // ALL
					SetMenuLevel(oPC, TTT, 1, 4);
					break;
			}
		} else {

			// Calculate amount by menulevel
			int nAmount = 0;
			switch ( nMenuLevel1 ) {
				case 1:
					nAmount = 1;
			...
			if ( 1 == nMenuLevel0 ) {
				...
				if ( bLimitedMoney && nMercMoney < nMercPaysHowMuch ) {
					...
					if ( !GetIsObjectValid(oThing) ) {
						...
						audit("sell", oPC, audit_fields("merchant", GetTag(oMerc), "resref", sSellsWhat,
								"price",
