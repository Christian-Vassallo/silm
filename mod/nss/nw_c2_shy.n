...
void main() {
	ExecuteScript("corpse_spawn", OBJECT_SELF);
	...
	//Rangers or Druids.
	//SetBehaviorState(NW_FLAG_BEHAVIOR_HERBIVORE);//Creature will flee those that close within 7m if they are enemies and no
	//Rangers or Druids.
	SetBehaviorState(NW_FLAG_BEHAVIOR_SHY);        //Creature will flee even neutral people unless they're rangers or druids.
	/*
	...
	ExecuteScript("os_" + GetStringLowerCase(GetTag(OBJECT_SELF)), OBJECT_SELF);
}
