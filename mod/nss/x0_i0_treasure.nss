...
const int BK_CHANCE_OF_N0_MONSTERTREASURE = 30;
...
//If set, don' convert to gold
int _dont_convert_items = 0;
...
//                              cloaks, helms, gloves)
object CTG_GetSpecificBaseTypeTreasureItem(object oBaseCont, int nItemNum, int nBaseType1 = BASE_ITEM_INVALID, int nBaseType2 = BASE_ITEM_INVALID,
...
//                              cloaks, helms, gloves)
//
void CTG_CreateSpecificBaseTypeTreasure(int nTreasureType, object oAdventurer, object oCont, int nBaseType1 = BASE_ITEM_INVALID, int nBaseType2 = BASE_ITEM_INVALID,
										int nBaseType3 = BASE_ITEM_INVALID);
...
int CTG_GetIsBaseType(int nItemBaseType, int nBaseType1 = BASE_ITEM_INVALID, int nBaseType2 = BASE_ITEM_INVALID,
					  int nBaseType3 = BASE_ITEM_INVALID);
...
void CTG_InitContainer(object oBaseCont) {
	// don't do this twice
	if ( CTG_GetIsContainerInitialized(oBaseCont) ) {
		return;
	}
...
int CTG_IsItemGold() {
	...
	if ( nRoll < TREASURE_GOLD_PROBABILITY && !_dont_convert_items )
		return TRUE;
...
void CTG_CreateSpecificBaseTypeTreasure(int nTreasureType, object oAdventurer, object oCont, int nBaseType1 = BASE_ITEM_INVALID, int nBaseType2 = BASE_ITEM_INVALID,
	...
	if ( CTG_GetIsTreasureGenerated(oCont) ) {
		return;
	}
...
void CTG_GenerateNPCTreasure(int nTreasureType = 5, object oNPC = OBJECT_SELF) {
	...
	// Otherwise, generate as usual, yet don't convert (creature/npc specific) items into money
	_dont_convert_items = 1;
	CTG_CreateTreasure(nTreasureType, oNPC, oNPC);
	_dont_convert_items = 0;

	if ( GetHitDice(oNPC) < 5 ) return;
	...
	//Chances are that the NPC carries some treasure appropiate to their level
	CTG_SetIsTreasureGenerated(oNPC, FALSE);
	if ( GetHitDice(oNPC) < 10 )
		CTG_CreateTreasure(TREASURE_TYPE_LOW, oNPC, oNPC);
	else if ( GetHitDice(oNPC) < 15 )
		CTG_CreateTreasure(TREASURE_TYPE_MED, oNPC, oNPC);
	else
		CTG_CreateTreasure(TREASURE_TYPE_HIGH, oNPC, oNPC);
}
