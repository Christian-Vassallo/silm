...
void main() {
	...
	string sOverrideName = GetLocalString(oPC, "omw_ovr_char");
	string sOverrideAcc  = GetLocalString(oPC, "omw_ovr_acc");
	SetLocalString(oPC, "omw_ovr_char", "");
	SetLocalString(oPC, "omw_ovr_acc", "");
	int nIsOverride = FALSE;

	if ( sOverrideName != "" && sOverrideAcc != "" ) {
		SendMessageToPC(oPC, "Account override in effect: " + sOverrideAcc + ":" + sOverrideName);
		nIsOverride = TRUE;
	} else {
		sOverrideName = GetName(oPC);
		sOverrideAcc  = GetPCPlayerName(oPC);
	}
	...
	if ( !GetIsObjectValid(oStorage) ) {
		...
		string sID = PPIS_DB_UNIQUE +
					 GetStringLeft(sOverrideAcc, iLength) + GetStringRight(sOverrideName, iLength);
