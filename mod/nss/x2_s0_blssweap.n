...
void AddBlessEffectToWeapon(object oTarget, float fDuration) {
	...
	// IPSafeAddItemProperty(oTarget, ItemPropertyEnhancementBonusVsAlign(IP_CONST_ALIGNMENTGROUP_EVIL,1), fDuration, X2_IP_ADDPROP_POLICY_KEEP_EXISTING,TRUE);
	// Replace existing temporary anti undead boni
	IPSafeAddItemProperty(oTarget, ItemPropertyDamageBonusVsRace(IP_CONST_RACIALTYPE_UNDEAD,
			IP_CONST_DAMAGETYPE_DIVINE, IP_CONST_DAMAGEBONUS_1d4), fDuration,
		X2_IP_ADDPROP_POLICY_KEEP_EXISTING);
	IPSafeAddItemProperty(oTarget, ItemPropertyDamageBonus(IP_CONST_DAMAGETYPE_MAGICAL,
			IP_CONST_DAMAGEBONUS_1), fDuration, X2_IP_ADDPROP_POLICY_IGNORE_EXISTING, TRUE, TRUE);
...
void main() {
	...
	if ( GetIsObjectValid(oTarget) && GetObjectType(oTarget) == OBJECT_TYPE_ITEM ) {
		...
		if ( GetBaseItemType(oTarget) ==  BASE_ITEM_BOLT ) {
			...
			IPSafeAddItemProperty(oTarget, ItemPropertyOnHitCastSpell(123, 1), RoundsToSeconds(nDuration),
				X2_IP_ADDPROP_POLICY_KEEP_EXISTING);
			ApplyEffectToObject(DURATION_TYPE_INSTANT, eVis, GetItemPossessor(oTarget));
			ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eDur, GetItemPossessor(oTarget), RoundsToSeconds(
					nDuration));
			return;
		}
	}
