...
// nVFX = VFX_BEAM_* constant
// fRotate = angle of rotation
void TextPigPenMessage(string sMessage, object oData, float fSpeed = 2.0f, float fLifetime = 0.0f, float fFontHeight = 0.5f, float fFontWidth = 0.25f, int nVFX = VFX_BEAM_FIRE_W_SILENT,
					   float fRotate = 0.0f);
...
void gao_CreateTextGrid(object oNode, float fFontHeight, float fFontWidth, object oData, float fLifetime,
	...
	for ( i = 0; i < 3; i++ ) {
		...
		for ( j = 0; j < 3; j++ ) {
			...
			oLedNode = CreateObject(OBJECT_TYPE_PLACEABLE, "invisobj", Location(oArea, vLedNode, fRotate),
						   FALSE, "PSC_X_TEXTMESSAGE");
			AssignCommand(oLedNode, ActionDoCommand(SetLocalObject(oNode, "led" +
						IntToString(j) + IntToString(i), oLedNode)));
			if ( fLifetime == 0.0 ) {
				nTotal = GetLocalInt(oData, "storetotal");
				AssignCommand(oLedNode, ActionDoCommand(SetLocalObject(oData, "store" +
							IntToString(nTotal), oLedNode)));
...
void TextMessage(string sMessage, object oData = OBJECT_SELF, float fSpeed = 2.0f, float fLifetime = 0.0f, float fFontHeight = 0.5f, float fFontWidth = 0.25f, int nVFX = VFX_BEAM_FIRE_W_SILENT,
	...
	for ( i = 0; i < nLength; i++ ) {
		...
		if ( oNode != OBJECT_INVALID ) {
			...
			DelayCommand(fSpeed *
				IntToFloat(i) +
				1.0, gao_AlphabetPermanent(oNode, GetSubString(sMessage, i, 1), nVFX, fLifetime));
		} else break;
	}
}

void TextPigPenMessage(string sMessage, object oData = OBJECT_SELF, float fSpeed = 2.0f, float fLifetime = 0.0f, float fFontHeight = 0.5f, float fFontWidth = 0.25f, int nVFX = VFX_BEAM_FIRE_W_SILENT,
					   float fRotate = 0.0f) {
	int i;
	object oNode;
	...
	for ( i = 0; i < nLength; i++ ) {
		...
		if ( oNode != OBJECT_INVALID ) {
			...
			DelayCommand(fSpeed *
				IntToFloat(i) +
				1.0, gao_PigPenAlphabetPermanent(oNode, GetSubString(sMessage, i, 1), nVFX, fLifetime));
		} else break;
	}
}
