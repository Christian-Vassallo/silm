...
public abstract class AbstractTlkReader<TlkTable>{
    ...
    public final TlkTable load(
        ...
        mbb.position(8);
        NwnLanguage language = NwnLanguage.forCode( mbb.getInt() );
        ...
        Charset charset = Charset.forName(language.getEncoding(nwnVersion));
        ...
        for ( int i = 0; i < entries; i++ ){
            ...
            if ( result.isError() ){
                System.err.printf("CharsetDecoder error on entry %d : %s\n",
                        i, result );
                // fall back to NWN1 encoding
                s = new String(strBuf,0,stringSizes[i],
                        language.getEncoding(Version.NWN1));
            }
            else
                s = cbuf.toString();
            createEntry( tlk, i,
