...
/**
  */
public class TlkLookupPanel extends JPanel{
	...
	private void setToStrRef(int strref) {
		posLabel.setText("StrRef " + strref);
		textArea.setText(tlkedit.tlkTable.getModel().getValueAt(strref, 2).toString());
	...
	public void lookup(int position) {
		...
		if ( isUserEntry(position) )
			...
			if (tlkedit == null && !isUserEntry(position))
				...
				textArea.setText(tlkedit.tlkTable.getModel().getValueAt(position, 2).toString());
	...
	private void lookupUserTlk( int position ){
		if ( usertlkedit == null )
			...
			if ( p > usertlkedit.tlkTable.getModel().getRowCount() - 2) {
				...
				textArea.setText(usertlkedit.tlkTable.getModel().getValueAt(p, 2).toString());
				textArea.setCaretPosition(0);
