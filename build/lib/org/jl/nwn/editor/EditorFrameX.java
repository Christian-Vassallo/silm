...
public class EditorFrameX extends JXFrame implements PropertyChangeListener{
    ...
    private FileFilter nw1filter = new FileFilter(){
        public boolean accept(File f) { return true; }
        public String getDescription() {
            return "All Files - NWN 1 mode";
        }
    };
    private FileFilter nw2filter = new FileFilter(){
        public boolean accept(File f) { return true; }
        public String getDescription() {
            return "All Files - NWN 2 mode";
        }
    };
    ...
    protected class OpenAction extends AbstractAction{
        ...
        public void actionPerformed(ActionEvent e) {
            if (tPane.getSelectedIndex() != -1)
                if (((SimpleFileEditor) tPane.getSelectedComponent()).getFile()
                != null)
                    fChooser.setCurrentDirectory(
                            ((SimpleFileEditor) tPane.getSelectedComponent())
                            .getFile()
                            .getParentFile());
            if (fChooser.showOpenDialog(navigator)
            == JFileChooser.APPROVE_OPTION) {
                final Version fileOpenVersion = openVersion != null ?
                    openVersion :
                    fChooser.getFileFilter() == nw1filter ?
                        Version.NWN1 : Version.NWN2;
    ...
    private Action actErfExtractForEditing = new AbstractAction(){
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            for ( int i = 0; i < ids.length; i++ ){
                try{
                    SimpleFileEditorPanel ed = openFile(
                            erf.extractAsTempFile( ids[i], true ), erf.getFileVersion() );
                    if (ed!=null){
        ...
        try {
            if (!f.exists()) {
                JOptionPane.showMessageDialog(
                        navigator,
                        Messages.getString("EditorFrame.ErrorMsgFileNotFound") + f.getAbsolutePath(), //$NON-NLS-1$
                        Messages.getString("EditorFrame.ErrorMsgTitle"), //$NON-NLS-1$
            ...
            if (TwoDaEdit.accept(f)) {
                ed = new TwoDaEdit(f, nwnVersion);
                ed.showToolbar(false);
                ((TwoDaEdit) ed).addListSelectionListener(
                        new TwoDaTlkLookupLSListener((TwoDaEdit) ed, tlp));
                ...
                JOptionPane.showMessageDialog(
                        navigator,
                        Messages.getString("EditorFrame.ErrorMsgUnknownFileType"),//$NON-NLS-1$
                        Messages.getString("EditorFrame.ErrorMsgTitle"),//$NON-NLS-1$
                        JOptionPane.ERROR_MESSAGE);
                return null;
            }
            ...
            JOptionPane.showMessageDialog(
                    navigator,
                    Messages.getString("EditorFrame.ErrorMsgCouldNotOpenFile") + ioex.getMessage(), //$NON-NLS-1$
                    Messages.getString("EditorFrame.ErrorMsgTitle"),//$NON-NLS-1$
                    JOptionPane.ERROR_MESSAGE);
            ioex.printStackTrace();
        }finally{
    ...
    private boolean closePane( JComponent c, boolean remove ) {
        ...
        if (sfe.getIsModified()) {
             ...
             */
            String optionMessage = MessageFormat.format(
                    Messages.getString("EditorFrame.WarningUnsavedChanges"), //$NON-NLS-1$
            ...
            int choice =
                    ...
                    tPane,
                    optionMessage,
                    Messages.getString("EditorFrame.WarningTitle_Unsaved"), //$NON-NLS-1$
    ...
    private void saveActivePane() {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(
                    navigator,
                    Messages.getString("EditorFrame.ErrorMsgCouldNotSaveFile"), //$NON-NLS-1$
                    Messages.getString("EditorFrame.ErrorMsgTitle"),//$NON-NLS-1$
                    JOptionPane.ERROR_MESSAGE);
            e.printStackTrace();
        }
    ...
    private void saveActivePaneAs() {
        ...
        fChooser.setFileFilter(
                ed.getFileVersion().equals(Version.NWN1) ?
                    nw1filter : nw2filter );
        do{
            ...
            if ( fcOption == JFileChooser.APPROVE_OPTION ){
                ...
                if ( f.exists() )
                    overwrite = JOptionPane.showConfirmDialog( this,
                            ...
                            Messages.getString("EditorFrame.FileExistsTitle"),
        ...
        if (fcOption != JFileChooser.APPROVE_OPTION)
            return;
        try {
            Version saveVersion = fChooser.getFileFilter() == nw1filter ?
                Version.NWN1 : Version.NWN2;
    ...
    private void updateTitle() {
        ...
        if (ed == null)
            ...
            setTitle(
                    ...
                    + " - " //$NON-NLS-1$
                    + (ed.getFile() != null
                    ? ed.getFile().getAbsolutePath()
                    : tPane.getTitleAt(tPane.getSelectedIndex()))
                    + " ("+ed.getFileVersion()+")"
                    + (ed.getIsModified() ?
                        Messages.getString("EditorFrame.TitleHint_FileModified")
                        : "")
                        ); //$NON-NLS-1$ //$NON-NLS-2$
    }
    ...
    public EditorFrameX() {
        ...
        Actions.registerActions(tPane.getInputMap(tPane.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT),
                tPane.getActionMap(), prevTab, nextTab);
        /*
        ...
        tPane.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent e) {
                SimpleFileEditorPanel ed =
                        (SimpleFileEditorPanel) ((JTabbedPane) e.getSource())
        ...
        ActionListener updateMemoryLabel = new ActionListener(){
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                statusBar.setLeadingMessage(
                        MessageFormat.format( msg, ((total - free) >> 10), (total >> 10) )
                        );
            }
        };
    ...
    private void restoreFromPreferences() {
        ...
        JDialog dialog = new JDialog(this, Messages.getString("EditorFrame.MsgRestoringSession"), false) { //$NON-NLS-1$
            {
        ...
        for (int i = 0; i < n; i++) {
            ...
            String fileVersion =
                    prefs.get("OpenFileVersion" + i, Version.getDefaultVersion().name()); //$NON-NLS-1$
            if (filename != null) {
        ...
        setSize(
                prefs.getInt("FrameWidth", 800), //$NON-NLS-1$
                prefs.getInt("FrameHeight", 600)); //$NON-NLS-1$
        dialog.dispose();
    ...
    private boolean checkUnsaved() {
        ...
        if (unsaved.size() > 0) {
            ...
            inputValues.add(
                    new JLabel(MessageFormat.format( Messages.getString("EditorFrame.MsgListOfUnsavedFiles"), Integer.valueOf(unsaved.size())))); //$NON-NLS-1$
            JCheckBox[] boxes;
            boxes = new JCheckBox[unsaved.size()];
            for (int i = 0; i < unsaved.size(); i++) {
                SimpleFileEditor sfe = (SimpleFileEditor) unsaved.get(i);
                boxes[i] =
                        new JCheckBox(
                        sfe.getFile() == null
                        ? Messages.getString("EditorFrame.FileNameUnsavedFile") //$NON-NLS-1$
            ...
            final JDialog dialog = new JDialog( this,
                    Messages.getString("EditorFrame.DialogTitleUnsavedFiles"), //$NON-NLS-1$
                    true );
            ...
            String[] optionLabels = new String[] {
                Messages.getString("EditorFrame.OptionSaveSelectedNExit"), //$NON-NLS-1$
                Messages.getString("EditorFrame.OptionExit"), //$NON-NLS-1$
            ...
            if (value == null || value == JOptionPane.UNINITIALIZED_VALUE || value == CANCEL ){
                ...
                for (int i = 0; i < boxes.length; i++)
                    ...
                    SimpleFileEditor sfe =
                            (SimpleFileEditor) unsaved.get(i);
    ...
    public void propertyChange(PropertyChangeEvent evt){
        if ( evt.getPropertyName() == SimpleFileEditorPanel.FILE_PROPERTY ||
                evt.getPropertyName() == SimpleFileEditorPanel.ISMODIFIED_PROPERTY ){
    ...
    
    {
        ...
        fChooser.addChoosableFileFilter(nw1filter);
        fChooser.addChoosableFileFilter(nw2filter);
        fChooser.setFileFilter(
                Version.getDefaultVersion().equals(Version.NWN1)?
                    nw1filter : nw2filter );
