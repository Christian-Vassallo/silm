...
#	  Last Modification : 19:55 Nov 23 2006 kivinen
#	  Last check in     : $Date: 2006/11/23 17:55:48 $
...
sub print_level {
	...
	if (!defined($$options{print_types}) ||
	    !$$options{print_types}) {
	    next if ($i =~
		     /____((struct_|file_|)type|file_version)$/);
	    next if ($i eq '');
	}
...
If both find_label and find_field is set, then both are checked, and
proc is only called if both match.
