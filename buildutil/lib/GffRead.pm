...
#	  Last Modification : 20:11 Nov 23 2006 kivinen
#	  Last check in     : $Date: 2006/11/23 18:17:58 $
...
sub read {
    ...
    my($file_type, $file_version, $struct_offset, $struct_count, 
	...
	return error(\%self, "Invalid version : $file_version");
    }
...
#   &Debug::debug(3, "File type = $file_type");
	...
	return error(\%self, "Struct array not after header, " .
		     "offset = $struct_offset instead of 56");
    }
    if (defined($options{'filename'})) {
	if (sysread(FILE, $data, $struct_len) != $struct_len) {
	    return error(\%self, "Could not read struct array");
	}
    } else {
...
#    &Debug::debug(5, "Struct array, offset = $struct_offset, len = $struct_len");
    ...
    if ($struct_count == 0) {
	return error(\%self, "No top level struct entry");
    }
	...
	return error(\%self, "Field array not after struct array, " .
		     ...
		     ($struct_offset + $struct_len));
    }
    if (defined($options{'filename'})) {
	if (sysread(FILE, $data, $field_len) != $field_len) {
	    return error(\%self, "Could not read field array");
	}
    } else {
...
#    &Debug::debug(5, "Field array, offset = $field_offset, len = $field_len");
	...
	if (sysread(FILE, $data, $label_len) != $label_len) {
	    return error(\%self, "Could not read label array");
	}
    } else {
