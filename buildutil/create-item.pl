...
#	  Last Modification : 18:41 Jan 23 2005 kivinen
#	  Last check in     : $Date: 2005/02/05 14:33:22 $
...
if ($#ARGV == -1) {
    ...
    $prop_id =
	$$CreateItem::des_crft_spells[$Opt::spell_id]{'IPRP_SpellIndex'};
	...
	$prop_id = $$CreateItem::des_crft_spells[$spell_id]{'IPRP_SpellIndex'};
...
sub create_one {
    ...
    if (!defined($name)) {
	$name = $$CreateItem::spells[$spell_id]{Label};
	$name =~ s/_/ /g;
	if ($Opt::type eq "p") {
	    $name = "Potion of " . $name;
	} else {
...
#	  "subtype = $subtype\n");
    $costvalue_ret = 0;
    $subtypecost_ret = 0;
    $propertycost_ret = $$CreateItem::itempropdef[$propertyname]{'Cost'};
    $propertycost_ret = 0 if ($propertycost_ret eq "****");
    if ($propertycost_ret == 0) {
	$subtyperesref2da =
	    $$CreateItem::itempropdef[$propertyname]{'SubTypeResRef'};
	if ($subtyperesref2da ne "****") {
	    $subtypecosttable =
		Twoda::read($Opt::twodadir . lc($subtyperesref2da) . ".2da");
	    $subtypecost_ret =
		$$subtypecosttable[$subtype]{'Cost'};
	}
    }
    $costtable2da = $$CreateItem::iprp_costtable[$costtable]{'Name'};
    if ($costtable2da ne "****") {
	$costtabletable = Twoda::read($Opt::twodadir .
				      lc($costtable2da) . ".2da");
	$costvalue_ret = $$costtabletable[$costvalue]{'Cost'};
    }
...
sub nwn_cost {
    ...
    $basecost = $$CreateItem::baseitems[$baseitem]{'BaseCost'};
    $basemult = $$CreateItem::baseitems[$baseitem]{'ItemMultiplier'};
    $multiplier = 0;
    $negmultiplier = 0;
    $spellcosts = 0;
    $first = 0;
