/***************************************************************************
    odmbc plugin for NWNX  - hooks implementation
...
int SCOproc(const int * pthis, char** database, char** key, char** player, int flags, unsigned char * pData, int size)
{
  if (*pthis)
  {
    ...
    asm ("pusha");
    int lastRet = (unsigned long)odmbc.WriteSCO(*database, *key, *player, flags, pData, size);
    asm ("popa");
    return lastRet;
  }
  return 0;
...
unsigned char * RCOproc(const int * pthis, char** database, char** key, char** player, int* arg4, int* size)
{
  if (*pthis)
  {
    ...
    asm ("pusha");
    unsigned char * lastRet = odmbc.ReadSCO(*database, *key, *player, arg4, size);
    asm ("popa");
    return lastRet;
  }
  return 0;
...
int HookFunctions()
{
  ...
  dword rco = FindHookRCO();
	if (rco)
	{
    odmbc.Log(0, "o RCO located at %x.\n", rco);
    d_redirect (rco, (unsigned long)RCOproc, d_ret_code_rco, 12);
	}
  else
  {
    odmbc.Log(0, "! RCO locate failed.\n");
    return 0;
  }
	return 1;
}
