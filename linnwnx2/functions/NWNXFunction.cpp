...
void CNWNXFunction::SetDescription(char* value)
{
	...
	if (iDescLength < length)
	{
		//FIXME Can only fill up to the length of the original desc
		strncpy(desc, value, iDescLength);
		*(desc+iDescLength) = 0x0;
	}
	else
	{
		//FIXME Possible memory leak?
		strncpy(desc, value, length);
		*(desc+length) = 0x0;
	}
}
...
void CNWNXFunction::SetBodyPart(char *value) 
{
	...
	if (*(pGameObject+0x4) == 0x5) // object type creature
	{
		...
		*(pGameObject+BODYPART_OFFSET+bpart) = (char)ptype;
	}
...
void CNWNXFunction::GetBodyPart(char *value) 
{
	...
	if (*(pGameObject+0x4) == 0x5) // object type creature
	{
		...
		sprintf(value,"%d",*(pGameObject+BODYPART_OFFSET+bpart));
	}
...
{
	printf("Object Dump: 0x%08lx\n",(unsigned long) pGameObject);
//	if (*(pGameObject+0x4) == 0x6) // object type item
	{
		int i,j;
		char buf[17];
