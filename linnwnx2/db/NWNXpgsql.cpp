...
bool CNWNXpgsql::ClientClose(const char *cname) {
	db->SqlDisconn();
	return true;
}

bool CNWNXpgsql::ClientInit(const char *cname, const char *dest) {
	...
	if(nwnxConfig->exists(confKey)) {
		...
		if((db = new DbPgSql(&dbParam))==NULL) {
			Log(0,"[%s] failed to create DbPgSql() object.\n",cname);
			return false;
		}
...
bool CNWNXpgsql::OnCreate(gline *config, const char *LogDir)
{
	// call the base class function
	char log[128];
	sprintf (log, "%s/nwnx_pgsql.txt", LogDir);
...
char *CNWNXpgsql::OnRequest(char *gameObject, char *Request, char *Parameters) {
	int len;
